# === characters.#albedo.credits ===

[Подробный разбор](https://keqingmains.com/albedo/) от Альбедо мейнеров и ККМ (англ.)


# === characters.#albedo.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#albedo.roles.#off-field_dps.tips ===

Элементальный навык Альбедо создаёт частицы энергии не во время активации способности, а в случайное время, пока стоит его Гео конструкция. Механика такая же, как и у Элементального навыка Чжун Ли, но цветок Альбедо создаёт больше частиц.


# === characters.#albedo.roles.#off-field_dps.notes ===

Альбедо своим Элементальным навыком призывает Гео конструкцию, которая наносит урон по противникам, которых атакует игрок в радиусе действия цветка, но не чаще раза в две секунды. Его Взрыв стихии даёт 125 Мастерства стихии, что полезно для отрядов играющих через реакции. Есть два классических билда Альбедо — полностью через Защиту, чтобы его Элементальный навык наносил максимальный урон, и гибридный, где к Защите добавляются статы на атаку для увеличения урона его Взрыва стихии.

**О выборе оружия**

[Драгоценный омут](#weapon:primordial-jade-cutter)<br>
При игре в гибридном билде это оружие может быть лучше, чем [Киноварное веретено](#weapon:cinnabar-spindle), в ситуациях когда Взрыв стихии Альбедо попадает по большому количеству врагов, или когда есть внешний бафф Силы атаки, например, от Беннета. При игре через Элементальный навык, или битве с одним противником, Киноварное веретено лучше.

[Предвестник зари](#weapon:harbinger-of-dawn)<br>
Элементальный навык Альбедо скейлится с Защиты, поэтому базовая атака оружия не имеет значения. Предвестник зари даёт много критов, поэтому это один из лучших мечей для Альбедо. Минус этого оружия в том, что нужно поддерживать ХП Альбедо выше 90%, что иногда может быть сложно, особенно против противников накладывающих Разложение.

[Рассекающий туман](#weapon:mistsplitter-reforged)<br>
Альбедо никогда не сможет собрать 3 стака бонуса этого оружия. Лучше отдать этот меч кому-то другому.

[Клятва свободы](#weapon:freedom-sworn)<br>
С этим оружием Альбедо значительно потеряет в личном уроне, зато будет хорошо баффать отряд благодаря тому, что легко может активировать бонус меча.

**О комплектах артефактов**

[Кокон сладких грёз (4)](#artifact:husk-of-opulent-dreams)<br>
Так как Элементальный навык Альбедо снапшотает, нужно подождать 12 секунд перед началом битвы, чтобы собрать все стаки бонуса комплекта. В Бездне время до начала испытания тоже считается.

[Tenacity of the Milelith (4)](#artifact:tenacity-of-the-millelith)<br>
С Альбедо, у этого комплекта хороший аптайм бонуса от четырех частей. Но с ним значительно падает урон его Элементального навыка.

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Если больше никто в отряде не носит этот комплект, то его можно выдать Альбедо. Ему понадобится немного допстатов на Восстановление энергии, чтобы чаще использовать Взрыв стихии. С этим комплектом значительно упадет урон Элементального навыка Альбедо.

**О приоритетных допстатах в артефактах**

Допстаты на Силу атаки и Восстановление энергии полезны только в гибридном билде.


# === characters.#aloy.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#aloy.roles.#burst_support.notes ===

Для Элой предпочтительно роль ульт саппорта, а не глав. ДД так как собрать на ней Крио инфузию достаточно сложно. Элой нужно собрать 4 стака, чтобы получить Крио инфузию и можно получать только 1 каждые 0.1 секунд, и это сложно, учитывая что бомбочки разбрасываются в случайные места. Но стоит обратить внимание, что Элой может накапливать стаки Спирали даже когда она в кармане.

Элой очень нишевая, и хорошо играется только в определенных отрядах. При этом в этих отрядах её всегда можно заменить на другого героя. Она может играть в отряде Заморозки, где она выполняет роль батарейки, обеспечивая 5 Крио частиц своим Элементальным навыком и при этом нанося большой урон умениями. Также она подходит для отряда Таяния, где вызов Таяния её Взрыва стихии нанесёт много урона.

**О выборе оружия**

[Полярная звезда](#weapon:polar-star)<br>
Нет смысла пытаться собирать стаки на этом оружии, это займет слишком много времени.

[Бесструнный](#weapon:the-stringless)<br>
Место в рейтинге предполагает, что Элой используется в отряде Реверсивного Таяния. В таком отряде, Бесструнный, особенно на высоком ранге пробуждения, может превзойти Громовой пульс.

[Прототип: Полумесяц](#weapon:prototype-crescent)<br>
Место в рейтинге основывается на том, что бонус оружия активен. Этот лук может быть так же хорош, как Небесное крыло, особенно на высоких рангах пробуждения.

[Церемониальный лук](#weapon:sacrificial-bow)<br>
Хороший вариант для Элой в роли Крио батарейки, потому что её Элементальный навык обеспечивает 5 Крио частиц.

**О комплектах артефактов**

Мастерство стихий нужно только в отряде, где Элой вызывает Реверсивное Таяние.

[Заблудший в метели (4)](#artifact:blizzard-strayer)<br>
Лучший комплект для отряда Заморозки, так как дает много Шанса крит. урона.

[Странствующий ансамбль (2)](#artifact:wanderers-troupe)<br>
Бонус этого комплекта подойдет только для Элой через Реверсивное Таяние, потому что Мастерство стихий не нужно в отряде заморозки.

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Этот комплект хорошо подходит Элой, потому что она может часто использовать свой Взрыв стихии.


# === characters.#amber.roles.#dps.name ===

ДД


# === characters.#amber.roles.#dps.notes ===

ДД Эмбер сосредоточена на максимизации урона от Заряженной атаки по слабым местам врага, так как пассивный талант, открывающийся на 60 уровне, увеличивает Силу атаки на 15% на 10 секунд после попадания в слабое место. Критическое попадание также гарантировано при попадании в слабое место, поэтому следует прокачать Крит. урон, практически игнорируя Шанс крит. попадания. Но это может измениться после \[С4\].

После \[С4\]**,** становится доступным стиль игры, ориентированный на урон от её навыков. В этом случае стоит экипировать Горящую алую ведьму (4), пытаясь сохранить соотношение Шанса крит. попадания к Крит. урону как 1:2 для более частых критических ударов. Этот стиль игры безопаснее, стабильнее и даёт больше урона по площади по сравнению со сборкой от Заряженной атаки.

**О комплектах артефактов**

[Воспоминания Симэнавы](#artifact:shimenawas-reminiscence)<br>
На \[С4\] этот сет больше подходит для глав. ДД сборки Эмбер через Заряженную атаку, чем Странствующий ансамбль.

**О выборе оружия**

[Прототип: Полумесяц](#weapon:prototype-crescent)<br>
Этот лук лучше, чем Небесное крыло, если он \[Р2+\].

[Хамаюми](#weapon:hamayumi)<br>
Этот лук лучше Клятвы стрелка \[Р5\] если он \[Р2+\].


# === characters.#amber.roles.#support.name ===

Саппорт


# === characters.#amber.roles.#support.notes ===

Из-за её плохих скейлов, этот раздел посвящен дешёвой и полезной сборке, которая позволяет ломать щиты благодаря быстрому наложению Пиро с помощью Взрыва стихии. Из-за того, что генерация энергии у Эмбер очень низкая, прокачка её Восстановления энергии приоритетна.

**О полном комплекте [Инструктора](#artifact:instructor)**

Этот набор предпочтительнее полного комплекта Церемонии древней знати, если Эмбер используется в качестве поддержки в команде испарения с Ху Тао.


# === characters.#ayaka.roles.#dps.name ===

ДД


# === characters.#ayaka.roles.#dps.subStats.notes ===

Шанс крит. попадания не в приоритете, так как предполагается использование [Заблудшего в метели (4)](#artifact:blizzard-strayer). Без этого набора Шанс крит. попадания так же важен, как и Крит. урон. В любом случае, нужно стремиться чтобы процент Крит. урона был в два раза выше Шанса крит. попадания.


# === characters.#ayaka.roles.#dps.tips ===

Заряженная атака Аяки, её Элементальный навык и Взрыв стихии достают до врагов внутри Взрыва стихии Венти.

Её Взрыв стихии прилипает к замороженным врагам и раскидывает не замороженных.


# === characters.#ayaka.roles.#dps.notes ===

Аяка простой в использовании ДД. Наносит больше всего урона своим Взрывом стихии. Скейлы её Взрыва стихии очень большие, и он стоит 80 энергии. Она предполагает отряд с Крио батарейкой, например Дионой, Кэйей, Розарией, и т.д. У нее особенный спринт, как у Моны, и после спринта она накладывает на свои обычные атаки Крио инфузию. Инфузию нужно все время поддерживать, для максимизации наносимого урона.

Она может играть в отряде постоянной заморозки, в отряде с Гидро персонажем, например, Моной, Син Цю, или даже Барбарой. В таком отряде она будет максимально использовать бонус комплекта Заблудшего в метели (4). Этот комплект крайне полезен в отряде Заморозки, поэтому следует собирать его как можно быстрее. Её Взрыв стихии липнет к замороженным врагам, но им сложно попасть, поэтому нужно быть внимательным при использовании.

**О выборе главстатов**

Выбор главстата короны зависит от общих статов персонажа. Корона на силу атаки может быть так же полезна как и крона на криты, так как Аяка может набрать достаточное количество критов со своего пассивного таланта и набора артефактов Заблудшего в метели и, возможно, со своего оружия. Кроме того, её проще добыть, благодаря более высокому шансу выпадения.

**О выборе оружия**

[Драгоценный омут](#weapon:primordial-jade-cutter)
С комплектом Заблудшего в метели (4), Крио резонансом и этим оружием получается слишком большой Шанс крит. попадания, поэтому следует избегать Шанса крит. попадания в допстатах.

[Меч Сокола](#weapon:aquila-favonia)
Не смотря на то, что главстат этого меча на физ. урон, что бесполезно для Аяки, он все еще полезен благодаря высокой Базовой атаке и баффу на процент Силы атаки. Но этот меч не превосходит другие четырехвездочные мечи, и для Меча Сокола может найтись владелец получше, например, Беннет.

[Небесный меч](#weapon:skyward-blade)
Благодаря бонусу Восстановления энергии этого меча, Аяка может чаще использовать взрыв стихии. С этим оружием рекомендуется избегать артефактов с допстатом на Восстановлении энергии, потому что Аяке для комфортной игры достаточно бонуса меча.

[Амэнома Кагэути](#weapon:amenoma-kageuchi)
Снижает потребность Аяки в Восстановлении энергии, что, в свою очередь, позволяет в допстатах артефактов взять больше атакующих статов. Кроме того, это оружие создается благодаря крафту, а не молитвам, что делает его легким в получении.

[Черногорский длинный меч](#weapon:blackcliff-longsword)
Так как Взрыв стихии Аяки снапшотает, бафф Атаки, полученный от убийства противника во время действия её Взрыва стихии, не увеличит урон этого взрыва.

[Предвестник зари](#weapon:harbinger-of-dawn)
Даёт хороший бафф Крита, если выполнять условие бонуса.

[Меч-флейта](#weapon:the-flute)
По урону может слегка превзойти Амэному Кагэути, но это будет стоить Восстановления энергии. Какой из этих двух мечей будет лучше — зависит от отряда.


# === characters.#ayato.roles.#dps.name ===

ДД


# === characters.#ayato.roles.#dps.artifacts.notes ===

\~= Конец гладиатора (4)


# === characters.#ayato.roles.#dps.tips ===

Следует всегда использовать Взрыв стихии Аято перед использованием его Элементального навыка, чтобы увеличить наносимый урон. Для этого нужно собрать достаточное количество Восстановления энергии, точное значение которого зависит от отряда.

Количество ударов, которых Аято может нанести во время действия Элементального навыка, иногда зависит от количества кадров в секунду, с которыми работает игра.


# === characters.#ayato.roles.#dps.notes ===

Роль Аято в команде как ДД состоит в нанесении большого урона Элементальным навыком и Взрывом стихии.

**О выборе оружия**

**[Небесный меч](#weapon:skyward-blade)**
при большом количестве Восстановления энергии в допстатах артефактов, это оружие становится равным [Амэноме Кагэути](#weapon:amenoma-kageuchi).

**[Кромсатель пиков](#weapon:summit-shaper)**
Когда недостаточно других баффов на атаку, это оружие может соперничать с Небесным мечом.

**[Драконий рык](#weapon:lions-roar)**
В командах построенных через реакцию Заряжен, этот меч лучше, чем [Чёрный меч](#weapon:the-black-sword).

**О комплектах артефактов**

**[Отголоски подношения (4)](#artifact:echoes-of-an-offering)**
Бонус этого комплекта зависит от пинга и других факторов.

**[Заблудший в метели (4)](#artifact:blizzard-strayer)**
Лучший комплект для игры через Заморозку. Не рекомендуется целенаправленно добывать этот комплект.

**[Громогласный рёв ярости (4)](#artifact:thundering-fury)**
Лучший комплект для игры через реакцию Заряжен, так как сокращает время отката Элементального навыка. Не рекомендуется целенаправленно добывать этот комплект.



# === characters.#barbara.roles.#dps.name ===

ДД


# === characters.#barbara.roles.#dps.tips ===

Из-за того, что Заряженные атаки Барбары потребляют много выносливости, рекомендуемый порядок атак для игры через реакцию Пар следующий: две обычные атаки, затем заряженная атака с прерыванием анимации прыжком.

В отрядах через реакцию Заряжен, Барбара должна вставлять свою атаку во время переключения между другими персонажами.


# === characters.#barbara.roles.#dps.notes ===

Этот билд посвещен игре через Пар и заряженные атаки. Для игры через Заряжен приоритет оружия такой же, как в Саппортском билде.

Крайне рекомендуется брать к Барбаре в отряд Сян Лин, так как она единсвенный персонаж, успевающий накладывать Пиро в нужном количестве.

Следует всегда использовать Элементальный навык, так как он снижает потребление выносливости, и на \[С2\] даёт 15% бонуса Гидро урона.

**О выборе оружия**

**[Память о пыли](#weapon:memory-of-dust)**<br>
Это оружие требует набивание стаков, и, при игре через Пар, для набивания стаков придется жертвовать частью реакций Пар.

**О комплектах артефактов**

**[Моллюск морских красок (4)](#artifact:ocean-hued-clam)**
Лучший комплект для игры через Заряжен. С этим комплектом следует собирать статы на лечение, а не личный урон. То есть, собирать артефакты на ХП, %ХП, Бонус лечения.

# === characters.#barbara.roles.#support.name ===

Саппорт

# === characters.#barbara.roles.#support.tips ===

Элементальное умение Барбары восстанавливает ХП активного персонажа каждые 5 секунд, при этом накладывает на него Гидро статус, поэтому его можно использовать для очищения элементального статуса.

При реакции с Крио персонаж будет заморожен. Это нужно учитывать при использовании умения.

# === characters.#barbara.roles.#support.notes ===

Следует сразу же переключаться после использования Элементального навыка или Взрыва стихии. Собирать много Восстановления энергии не рекомендуется, так как Взрыв стихии Барбары используется лишь в крайней необходимости, а не по откату способности. Кроме того, Барбара не генерирует частички. Поэтому **[Моллюск морских красок (4)](#artifact:ocean-hued-clam)** не рекомендуется для Барбары в роли Саппорта.

**О выборе оружия**

**[Everlasting Moonglow](#weapon:everlasting-moonglow)**
При игре через Заряжен, с этим оружием у Барбары будет максимальный собственный урон. Но с [Эпосом о драконоборцах](#weapon:thrilling-tales-of-dragon-slayers) общий урон отряда будет выше из-за баффа на 48 процентов атаки.

**О комплектах артефактов**

**[Возлюбленная юная дева (2) ](#artifact:maiden-beloved)**
Не рекомендуется специально добывать этот комплект для Барбары.

**[Инструктор (4)](#artifact:instructor)**
Наложение Гидро Элементальным навыком Барбары может вызвать элементальную реакцию и баффнуть отряд на Мастерство стихий.


# === characters.#beidou.credits ===

[Подробный разбор](https://keqingmains.com/beidou/) от Бэй Доу мейнеров и ККМ (англ.)


# === characters.#beidou.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#beidou.roles.#off-field_dps.subStats.notes ===

В первую очередь, стоит прокачивать Восстановление энергии до тех пор, пока не получится ультовать по откату.


# === characters.#beidou.roles.#off-field_dps.tips ===

Элементальное умение Бэй Доу накладывает на короткое время на неё Электро статус. Его можно использовать для очистки других статусов с себя, но со слабой эффективностью. Так можно очистить статус только с Бэй Доу.


# === characters.#beidou.roles.#off-field_dps.notes ===

**О выборе оружия**

**[Меч драконьей кости](#weapon:serpent-spine)**<br>
Это оружие можно предпочесть Волчьей погибели для доп. ДД Бэй Доу, а Волчью погибель экипировать на другого двуручника с бо́льшим временем активности на поле. Стаки баффа этого оружия могут эффективно использоваться Бэй Доу, если набить все стаки перед переключением с нее на другого персонажа, так как эффект стаков будет активен всё время действия её Взрыва стихии. **Меч драконьей кости значительно превосходит Волчью погибель, когда активны баффы Беннета или Сары на Силу атаки.**

**[Краснорогий камнеруб](#weapon:redhorn-stonethresher)**<br>
В зависимости от получаемых баффов на Силу атаки, это оружие может превзойти Волчью погибель.

**[Акуомару ](#weapon:akuoumaru)(в отряде с суммарной энергией 260)**<br>
Акуомару превосходит **Небесное величие** на  **Бэй Доу, которой не нужно высокое Восстановление энергии, то есть когда она в команде с Электро батарейкой.** На \[Р3+\], оно становится значительно лучше приближается к **Мечу драконьей кости** и **Волчьей погибели.**

**О комплектах артефактов**

**[Эмблема рассечённой судьбы (4)](#artifact:emblem-of-severed-fate)**<br>
Не нужно собирать **Восстановления энергии** больше, чем нужно для отката её Взрыва стихии, так как это приведет к потере наносимого урона. В отряде, где Бэй Доу в команде с Электро батарейкой, Церемония древней знати (2) или Громогласный рёв ярости (2) или +18 процентов Силы атаки (2), **любые два комплекта лучше,** чем **Эмблема рассечённой судьбы (4).**

**[Усмиряющий гром (4)](#artifact:thundersoother)**<br>
Лучший комплект для отрядов, построенных на реакции Заряжен, или в моно Электро командах.

**[Церемония древней знати (4)](#artifact:noblesse-oblige)**
Этот комплект может быть рассмотрен, если в отряде больше никто не носит Церемонию древней знати (4). Следует обратить внимание, что сама Бей Доу иногда не получает бонус от этого комплекта, если она является его владельцем, поэтому он не рекомендуется.

**[Учёный (2) ](#artifact:scholar)+ [Изгнанник (2)](#artifact:the-exile)**<br>
Этот комплект — дешевый вариант для начинающих, помогающий поддерживать аптайм Врыва стихии Бэй Доу.


# === characters.#bennett.credits ===

[Подробный разбор](https://keqingmains.com/bennett/) от ККМ (англ.)


# === characters.#bennett.roles.#dps.name ===

ДД


# === characters.#bennett.roles.#dps.mainStats.notes ===

Обычно бонус Мастерства стихий в часах приоритетнее процента Атаки, но окончательный выбор зависит от допстатов и оружия.


# === characters.#bennett.roles.#dps.subStats.notes ===

Процент атаки важнее Мастерства стихий при использовании [Стального жала](#weapon:iron-sting)


# === characters.#bennett.roles.#dps.talents.notes ===

Имеет смысл прокачивать обычную атаку только если есть С6 Беннет.


# === characters.#bennett.roles.#dps.tips ===

Взрыв стихии Беннета накладывает на персонажа внутри области действия Пиро статус.

Поэтому, если персонаж, на которого наложен эффект любой стихии, зайдет в область Взрыва стихии Беннета, то на него наложится Пиро, и будет вызвана соответствующая реакция. Таким образом можно снимать с персонажа нежелательные статусы. Стоит учитывать, что враги при атаке персонажа, на котором наложено Пиро, так же вызывают реакцию и наносят увеличенный урон.


# === characters.#bennett.roles.#dps.notes ===

**О выборе оружия**

[Вспышка во тьме](#weapon:the-alley-flash)<br>
Вспышка во тьме может быть лучше, чем Стальное жало \[Р5\]**,** но только при условии, что ее пассивная способность будет всегда активна. После получения урона пассивка пять секунд не работает и теряется много урона. Это особенно критично, если персонаж все время получает урон.

**О комплектах артефактов**

[Горящая алая ведьма (4)](#artifact:crimson-witch-of-flames)<br>
Этот комплект лучше всего работает с Чун Юнем из-за постоянного Таяния.<br>
В отряде, построенном на реакции **Таяния/Пара,** Горящая алая ведьма (4) лучший комплект для урона.<br>
Кубок на бонус Пиро урона всегда лучший выбор. Лучшее сочетание для Таяния/Пара — Чёрный меч и часы на Мастерство стихий. \[Р1\] Стальное жало и часы на Мастерство стихий часы наносят меньше урона, но \[Р2+\] Стальное жало это другое дело из-за более сильного баффа. В случае, если у удалось собрать невероятно большие Шансы крита и Крит. урона, то Меч Сокола с часами на Мастерство стихий бесспорно лучше

[Громогласный рёв ярости (4)](#artifact:thundering-fury)<br>
Этот комплект **работает лучше всего** если находиться **в паре с Электро саппортами,** например, с Бэй Доу, Фишль или Лизой, что позволяет достичь Беннету две вещи: <br>
1) все время вызывать Перегрузку своим элементальным навыком<br>
2) генерировать тонну энергии для всего отряда <br>
Громогласный рёв ярости (4) так же позволяет Беннету **быстро ломать щиты Чтецов Бездны.** Эта команда рекомендована только с Бэй Доу на С2/С6.

Следует обратить внимание, что главстаты артефактов, которые собираются в Громогласный рёв ярости (4) отличаются от главстатов артефактов, которые обычно собираются в других комплетах. Вместо Силы атаки или Мастерства стихий (часы), Пиро (кубок), и Критов (корона), следует собрать Мастерство стихий, Пиро урон, Мастерство стихий. Также рекомендуется использовать оружие с допстатами на Мастерство стихий, например Стальное жало.

Стоит так же отметить, что с комплектом Громогласный рёв ярости (4), Драгоценный омут может превзойти Рассекающий туман из-за его большого допстата на Шанс крит. попадания. Но только в случае, если в кусках на Мастерство стихий (часах, кубке и короне) недостаточно допстатов на Шанс крит. попадания. Однако, в тех редких случаях, когда в куски на Мастерство стихий выпало достаточно критов, Рассекающий туман легко превзойдет Драгоценный омут.

Рекомендованое количество процента Восстановление энергии: 130-150%. Стоит отметить, что с Громогласным рёвом ярости (4), процент Восстановления энергии может быть меньшим.


# === characters.#bennett.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#bennett.roles.#burst_support.subStats.notes ===

В первую очередь, стоит прокачивать Восстановление энергии до тех пор, пока не получится ультовать по откату.


# === characters.#bennett.roles.#burst_support.tips ===

Рекомендуемой процент Восстановления энергии: 180-200


# === characters.#bennett.roles.#burst_support.notes ===

Ульт-саппорт Беннет **полностью игнорирует необходимость дополнительного лечения** и нацелен на тех, кто хочет выжать из него больше урона. В этой сборке Беннета предпочтительнее атакующие статы, такие как процент Силы атаки, Пиро урон и Криты, и при этом достаточное количество Восстановления энергии, чтобы ультовать по откату.<br>
**Если не удается ультовать по откату, следует сфокусироваться на Восстановлении энергии.**


# === characters.#bennett.roles.#support.name ===

Саппорт


# === characters.#bennett.roles.#support.mainStats.notes ===

Главстаты не очень важны. Лучше собрать достаточно Восстановления энергии для частого Взрыва стихии.


# === characters.#bennett.roles.#support.tips ===

Рекомендуемое количество Восстановления энергии: 200+


# === characters.#bennett.roles.#support.notes ===

**Лечение от** его Взрыва стихии **зависит от его ХП,** в то время, как **бафф урона** от Взрыва стихии рассчитывается **только от Базовой атаки,** а именно Базовой атаки персонажа и Базовой атаки оружия.

**НЕ СЛЕДУЕТ увеличивать созвездие Беннета с 5 на 6,** если не уверен в необходимости. Потому, что это созвездие **преобразует все обычные атаки в области Взрыва стихии Беннета в Пиро,** из-за чего Беннет теряет гибкость в выборе отряда.

Хотя Беннет часто рассматривается как целитель, он хорошо лечит даже если не вкачивать его ХП, поэтому лучше собирать его в доп. ДД.

**О выборе оружия**

[Рассекающий туман](#weapon:mistsplitter-reforged)**,**[ Меч Сокола](#weapon:aquila-favonia)<br>
У этого оружия самая высокая Базовая атака в игре, поэтому оно так высоко оценено. Предполагается, что Беннет имеет достаточно Восстановления энергии, чтобы ультовать по откату для максимального баффа Силы атаки.

[Клятва свободы](#weapon:freedom-sworn)<br>
В командах, где Беннет может часто вызывать реакции и таким образом активировать бафф меча, бафф этого оружия будет лучше, чем бафф Меча Сокола.

[Вспышка во тьме](#weapon:the-alley-flash)<br>
У этого оружия на данный момент наибольшая Базовая атака из всего 4 звездочного оружия. Даже больше некоторых 5 звездочных, например, больше Небесного меча.

[Прототип: Злоба](#weapon:prototype-rancour)<br>
Базовая атака этого оружия одна из наибольших среди 4 звездочных. Так же этот меч можно выковать у кузнеца.


# === characters.#chongyun.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#chongyun.roles.#burst_support.tips ===

Взрыв стихии Чун Юня считается уроном двуручного меча. Это значит, что он быстро разбивает Гео структуры и щиты, а так же может разбивать замороженных противников.


# === characters.#chongyun.roles.#burst_support.notes ===

Чун Юнь предпочтительнее в роли доп. ДД, чем глав. ДД, так как скейлы его обычной атаки очень низкие, даже когда он \[С1\]. Его Взрыв стихии наносит много урона на больших созвездиях и является его основным источником урона. Кроме того, всеми тремя ударами его Взрыва стихии можно вызывать реакцию Реверсивного Таяния, если хорошо наложить Пиро, например, Элементальным навыком и Взрывом стихии Беннета.

Чун Юнь очень нишевый, так как он работает хорошо только в определенных отрядах и может полностью сломать другие. Обычно он используется в отрядах Реверсивного Таяния или заморозки. Он не играется в Физ. отрядах, например с Рэйзором или Эолой, так как не может использовать свой Элементальный навык, так как он превращает урон их обычных атак с физических в Крио. Чаще всего они используют Физ. кубок, поэтому если их обычная атака преобразуется с физической в Крио, они не получат преимуществ бонуса Физ. урона с кубка, это приведет к потерям урона.

На \[С2\] Элементальный навык уменьшает время отката умений, которые были активированы в его радиусе, на 15%. Это одна из немногих способностей в игре, уменьшающая время отката.

**О выборе оружия**

[Меч драконьей кости](#weapon:serpent-spine)<br>
Это оружие лучше, чем [Волчья погибель](#weapon:wolfs-gravestone), но только для собственного урона Чун Юня, и требует накопления пяти стаков. [Волчья погибель](#weapon:wolfs-gravestone) оценена выше, потому что обеспечивает увеличенный урон всему отряду и не требует накопления стаков для битвы.

[Краснорогий камнеруб](#weapon:redhorn-stonethresher)<br>
В зависимости от количества наложенных на Чун Юня баффов Силы атаки, это оружие может превзойти [Вольчю погибель](#weapon:wolfs-gravestone) по собственному урону Чун Юня.

[Акуомару](#weapon:akuoumaru)<br>
На \[Р1\], Акуомару лучше, чем [Благодатный владыка вод](#weapon:luxurious-sea-lord), если Чун Юнь используется в отряде с 260 или выше общей суммой стоимости Взрывов стихии. [Акуомару](#weapon:akuoumaru) лучше, чем [Благодатный владыка вод](#weapon:luxurious-sea-lord) только на \[Р2+\].

[Церемониальный двуручный меч](#weapon:sacrificial-greatsword)<br>
Это оружие — лучший выбор для ситуации, где крайне важен аптайм наложения на атаки Крио инфузии Элементальным навыком Чун Юня.

**О комплектах артефактов**

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Чун Юнь — один из лучших носителей Церемонии, благодаря тому, что у него низкая потребность в энергии для Взрыва стихии.

**О приоритетах талантов**<br>
Взрыв стихии приоритетен в отрядах Таяния, так как он склейлится лучше Элементального навыка.<br>
Элементальный навык приоритетен в отрядах Заморозки, так как повышает длительность Крио инфузии.


# === characters.#diluc.roles.#dps.name ===

ДД


# === characters.#diluc.roles.#dps.subStats.advices.2.notes ===

приоритетнее в командах Испарения и Таяния


# === characters.#diluc.roles.#dps.talents.notes ===

Элементальный навык становится приоритетнее с ростом уровня созвездия.


# === characters.#diluc.roles.#dps.tips ===

Можно вставлять две Обычные атаки (ОА) между атаками Элементального навыка (ЭН).

То есть, атаковать так: ЭН -> ОА -> ОА -> ЭН -> ОА -> ОА -> ЭН -> ОА -> ОА


# === characters.#diluc.roles.#dps.notes ===

**О выборе оружия**

[Краснорогий камнеруб](#weapon:redhorn-stonethresher)<br>
В зависимости от количества баффов на процент Силы атаки, это оружие может обогнать Волчью погибель по урону.

[Меч драконьей кости](#weapon:serpent-spine)<br>
Так как бонус \[С2\] Дилюка требует получения урона для баффа Силы атаки, это приводит к потере урона из-за сброса баффа Меча драконьей кости. Но в команде с **Чжуном Ли,** щит позволяет получить бафф от \[С2\] без потери стаков — поэтому настоятельно рекомендовано использование Чжуна Ли с \[С2\] Дилюком.

[Черногорская бритва](#weapon:blackcliff-slasher)<br>
У этого оружия прекрасный ДПС потенциал при столкновении с толпой врагов, но в это же время потенциал теряется при битве с одиночной целью.

[Дождерез](#weapon:rainslasher)<br>
По очевидным причинам, это оружие рекомендовано только когда Дилюк находится в отрядах, где он может регулярно вызывать Пар, то есть когда в паре с Син Цю. **Дождерез** так же **обгонит [Небесное величие](#weapon:skyward-pride) на \[Р3+\].** Главстат Дождереза даёт бонус Мастерства стихий, так что с ним рекомендуется использовать часы на процент Силы атаки.

**О комплектах артефактов**

[Горящая алая ведьма (4)](#artifact:crimson-witch-of-flames)<br>
В целом, лучший комплект для Дилюка благодаря тому, что он часто может наносить Пиро урон.

[Конец гладиатора (4)](#artifact:gladiators-finale)<br>
Для тех, кто еще не собрал комплект Горящей алой ведьмы, этот набор предоставит заметное увеличение урона, так как баффает урон обычных атак, а Дилюк играется в основном через обычные атаки.

**О выборе главстатов**

Если Дилюк находится в команде с Син Цю, где он очень часто может вызывать Пар, можно взять часы на Мастерство стихий часы вместо часов на Силу атаки. Мастерство стихий значительно увеличивает урон реакций. Обратите внимание, что когда после Взрыва стихии атаки Дилюка бьют Пиро инфузией, то он может вызывать Пар каждые две обычные атаки из-за внутреннего времени отката.


# === characters.#diona.roles.#support.name ===

Саппорт


# === characters.#diona.roles.#support.tips ===

Элементальный навык Дионы накладывает на активного персонажа Крио статус при переключении. Это можно использовать для снятия с себя статусов. Стоит обратить внимание, что накладывание Крио на персонажа с Гидро статусом заморозит его.


# === characters.#diona.roles.#support.notes ===

Диона-щитовик не должна фокусироваться на лечении. Бонус урона от Церемонии древней знати (4) гораздо более ценен. Также её первое созвездие значительно увеличивает аптайм ее Взрыва стихий, дополнительно снижая важность лечения.

Как саппорт, Диона используется для трех вещей — щит, лечение и генерация частиц. Диона в отряде с Крио ДД, например, Эолой или Гань Юй, предоставляет им защиту и служит Крио батарейкой. Церемониальный лук и Боевой лук Фавония значительно увеличивают её возможности батарейки.

**О выборе оружия**

**[Церемониальный лук](#weapon:sacrificial-bow)**<br>
Предпочтительнее, чем Боевой лук Фавония, особенно на высоких рангах пробуждения, благодаря возможности чаще использовать щит.

**[Боевой лук Фавония](#weapon:favonius-warbow)**<br>
Под него придётся собирать билд с хорошим Шансом крит. попадания для повышения шанса активации бонуса лука.

**[Изогнутый лук](#weapon:recurve-bow)**<br>
Хороший выбор усиления щитов, но с ним Диона будет реже использовать Взрыв стихии.

**О комплектах артефактов**

**[Церемония древней знати (4)](#artifact:noblesse-oblige)**<br>
Комплект даёт бонус атаки отряду и бонус урона собственного Взрыва стихии. Церемония древней знати обычно используется на лекарях, потому что на других персонажах поддержки, например, на Син Цю или на Бэй Доу, полезнее комплекты на урон.

**[Эмблема рассечённой судьбы (2) ](#artifact:emblem-of-severed-fate)+ [Стойкость Миллелита (2)](#artifact:tenacity-of-the-millelith)**<br>
Дают и Восстановление энергии, и ХП, что полезно для Дионы-саппорта, потому что с ними она чаще использует Взрыв стихии при этом имет более крепкий щит.

**О выборе главстатов**

Собирать всё на ХП рекомендуется, когда нужны более сильные щиты и не так важно лечение. Если взять корону на лечение, получится баланс между щитами и лечением.

**О приоритетах талантов**<br>
Приоритет улучшения талантов зависит от того, для чего Диона используется в отряде — щитов или лечения. Если нужно всё сразу, то и улучшать нужно всё сразу.


# === characters.#eula.roles.#dps.name ===

ДД


# === characters.#eula.roles.#dps.tips ===

**Последовательность атак вне Взрыва стихии**<br>
Спамить ОА > быстро нажать Элементального навыка > повторить

**Последовательность атак во время Взрыва стихии**<br>
быстрое нажатие Элементального навыка > Взрыв стихии > 4 ОА > долгое нажатие Элементального навыка > 4 ОА

**Последовательность атак во время Взрыва стихии, которая генерирует больше энергии и наносит чуть меньше урона**<br>
быстрое нажатие Элементального навыка > Взрыв стихии > быстрое нажатие Элементального навыка > 4 ОА > сорвать ротацию прыжком или спринтом > 2 ОА > долгое нажатие Элементального навыка > 2 ОА

\*ОА — обычная атака


# === characters.#eula.roles.#dps.notes ===

Рейтинг оружия для Эолы основывается на том, что она играет с Крио батарейкой вместо Беннета. Ей нужно 30%-60% Восстановления энергии в зависимости от того, какая батарейка используется.

Взрыв стихии Эолы не снапшотает. Этот означает, что какие бы баффы не были на ней во время активации Взрыва стихии, они не повлияют на взрыв Меча света, если закончатся к моменту взрыва.

**О выборе оружия**

[Краснорогий камнеруб](#weapon:redhorn-stonethresher)<br>
В зависимости от количества баффов Силы атаки у Эолы, это оружие может превзойти Волчью погибель по личному урону.

[Небесное величие](#weapon:skyward-pride)<br>
Это оружие теряет свою эффективность, если Эола в команде с хорошей батарейкой, например Дионой с Церемониальным луком или Кэйей с Мечом Фавония, или если отряд хорошо генерирует частицы сам по себе.

[Акуомару](#weapon:akuoumaru)<br>
Это оружие превзойдет Благодатного владыку вод на высоких рангах пробуждения.

**О комплектах артефактов**

[Бледный огонь (4)](#artifact:pale-flame)<br>
Когда активен бонус комплекта, то Бледный огонь (4) лучше, чем набор Бледный огонь (2) Рыцарь крови (2). Если всегда поддерживать бафф, Эола получит 50% бонус физ. Урона и 18% Силы атаки. С этим комплектом рекомендуется использовать долгое нажатие Элементального умения перед самым взрывом Меча света. Это касается тех, у кого Эола не \[С2+\].

[Бледный огонь (2)](#artifact:pale-flame) + [Рыцарь крови (2)](#artifact:bloodstained-chivalry)<br>
Этот комплект без каких либо условий дает постоянный 50% бонус физ. Урона. Но Бледный огонь (4) может легко превзойти этот набор, потому что Эола легко активирует его бонус благодаря быстрому откату её Элементального навыка.


# === characters.#fischl.roles.#dps.name ===

ДД


# === characters.#fischl.roles.#dps.tips ===

[Более подробный разбор оружия (англ.)](https://bit.ly/3xOMZyh)

Учитывая то, как работает Полярная звезда, оптимальная последовательность атак будет отличаться от другого оружия:

**Элементальный навык (или Взрыв стихии) > обычная атака > Заряженная Атака > Перевызов Оза (чтобы наложить на него бонус от стаков Полярной звезды)**


# === characters.#fischl.roles.#dps.notes ===

Без \[С1\] **не рекомендуется собирать** Фишль в ДД.

У ДД Фишль есть техника отмены анимации. После трех или пяти обычных атак, нужно выполнить прицельный выстрел, чтобы сбросить комбо. Чем быстрее получится сделать прицельный выстрел — тем лучше. Эту технику стоит применять только когда закончилась выносливость, так как она достаточно сложная в исполнении.

**О \[С6\] Фишль сборке**

На **\[С6\]** Оз будет наносить 60% урона, поэтому, пока нет возможности собрать Бледный огонь (4) с Электро Кубком, можно использовать **Конец гладиатора (2) и Громогласный рёв ярости (2).**

**О выборе оружия**

**[Небесное крыло ](#weapon:skyward-harp)и [Полярная звезда](#weapon:polar-star)**<br>
Из-за  упомянутого выше соотношения наносимого Фишль и Озом урона на **\[С6\], Полярная звезда** будет лучше, чем **Небесное крыло.**

**[Зелёный лук](#weapon:the-viridescent-hunt)**<br>
Зелёный лук хорошая инвестиция, даже превосходящая Амос в ситуациях, где много врагов, хотя он не так эффективен против массивных или быстродвижущихся целей. Он дает урон по площади, которого не хватает Фишль, ввиду того, что она бьет только по одной цели.

**[Вальс Нирваны Ночи](#weapon:mitternachts-waltz)**<br>
Для физ. ДД Фишль Вальс Нирваны Ночи стоит предпочесть Амосу, если удастся собрать ему \[Р5\].

**[Прототип: Полумесяц](#weapon:prototype-crescent)**<br>
Если делать заряженный выстрел по слабому месту противника каждые 9-10 секунд, то Прототип: Полумесяц сможет дать больше урона. Это оружие может подойти тем, кому нравится более сложный стиль игры.


# === characters.#fischl.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#fischl.roles.#off-field_dps.notes ===

И Взрыв стихии, и Элементальный навык призывают Оза, поэтому нужно использовать их по очереди.

**О выборе оружия**

**[Бесструнный](#weapon:the-stringless) и [Охотник во тьме](#weapon:alley-hunter)**<br>
На \[Р5\] оба этих оружия равны [Громовому пульсу](#weapon:thundering-pulse) и [Небесному крылу](#weapon:skyward-harp).

**[Прототип: Полумесяц](#weapon:prototype-crescent)**<br>
Ранг предполагает выполнение заряженной атаки в слабое место противника каждый раз перед вызовом Оза.

**[Ода анемонии](#weapon:windblume-ode)**<br>
Ода хороша, только когда Фишль вызывает реакции. Если Фишль состоит в отряде, где нет реакций, то это оружие хуже, чем Вальс Нирваны Ночи.

**[Вальс Нирваны Ночи](#weapon:mitternachts-waltz)**<br>
Это оружие на \[Р5\] лучше, чем Ода анемонии.

**О комплектах артефактов**

**[Усмиряющий гром (4)](#artifact:thundersoother)**<br>
Этот комплект артефактов превосходит Конец гладиатора (2) Громогласный рёв ярости (2) в отрядах, где на противнике всегда наложено Электро, и является лучшим выбором в данном случае.

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)**<br>
Этот комплект обменивает урон Фишль на 20 процентов баффа Силы атаки для всего отряда. Не рекомендовано добывать этот комплект специально для Фишль.


# === characters.#ganyu.credits ===

[Подробный разбор](https://keqingmains.com/ganyu/) от Гань Юй мейнеров и ККМ (англ.)


# === characters.#ganyu.roles.#melt_dps.name ===

ДД через Таяние


# === characters.#ganyu.roles.#melt_dps.tips ===

В некоторых командах и ситуациях Взрыв стихий Гань Юй может негативно сказаться на общем уроне: взрыв поглотит ауру Пиро, и заряженной атакой реакцию Таяния вызвать не получится.


# === characters.#ganyu.roles.#melt_dps.notes ===

Второй уровень заряженного выстрела Гань Юй накладывает два Крио, и, так как у них отдельные внутренние таймеры реакций, такой выстрел может вызывать реакцию Таяние дважды. Заряженная атака — основной источник её урона.

**О выборе оружия**

[Лук Амоса](#weapon:amos-bow)<br>
Амос это прекрасное оружие для Гань Юй так как 2 уровень её заряженной атаки увеличивает время полета стрелы, и, даже в ближнем бою, дает 3 стака пассивки.

[Хамаюми](#weapon:hamayumi)<br>
Этот оружие не работает с Воспоминаниями Симэнавы (4) из-за своего бонуса.

[Прототип: Полумесяц](#weapon:prototype-crescent)<br>
На высоких уровнях пробуждения с хорошим аптаймом бонуса это оружие может превзойти Небесное крыло. Но стоит помнить, что не у всех врагов есть слабые места.

**О приоритетных талантах**

Взрыв стихий Гань Юй не очень важен в команде Таяния: он будет мешать из-за поглощения Пиро.


# === characters.#ganyu.roles.#freeze_dps.name ===

ДД через Заморзку


# === characters.#ganyu.roles.#freeze_dps.tips ===

Не нужно собирать слишком большой Шанс крит. попадания. 25%-45% достаточно, остальные проценты соберутся от Крио резонанса, бонуса артефактов и её пассивного умения, повышающего Шанс крит. попадания заряженного выстрела.


# === characters.#ganyu.roles.#freeze_dps.notes ===

Взрыв стихии Гань Юй может нанести тонну урона, если враги будут стоять достаточно кучно, так как каждая упавшая сосулька наносит урон по площади.

**О выборе оружия**

**[Полярная звезда](#weapon:polar-star)**<br>
С этим оружием нужно внимательно собирать артефакты, чтобы Шанс крит. попадания не стал выше 100%. С правильными статами артефактов это оружие превзойдет другие варианты.

**[Лук Амоса](#weapon:amos-bow)**<br>
В отрядах, где Гань Юй использует только навыки и не стреляет, лучше себя покажут другие пятизвездочные оружия.

**[Прототип: Полумесяц](#weapon:prototype-crescent)**<br>
Как и в билде через Таяние, Прототип на верхних уровнях пробуждения и с постоянным аптаймом бонуса может превзойти некоторые пятизвёздочные варианты вроде [Громового пульса](#weapon:thundering-pulse) или [Небесного крыла](#weapon:skyward-harp). Но стоит помнить, что не у всех врагов есть слабые места.

**О комплектах артефактов**

**[Заблудший в метели (4)](#artifact:blizzard-strayer)**<br>
Является стандартным комплектом для отряда Заморозки благодаря своим мощным бонусам.


# === characters.#ganyu.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#ganyu.roles.#off-field_dps.subStats.advices.0.notes ===

собирать, пока взрыв стихии не будет по откату


# === characters.#ganyu.roles.#off-field_dps.subStats.advices.3.notes ===

не для отряда заморозки


# === characters.#ganyu.roles.#off-field_dps.tips ===

После активации Взрыва стихии, рекомендуется использовать её Элементальный навык внутри радиуса Взрыва, провоцируя врагов оставаться внутри и получать урон.

И при установке, и при уничтожении цветок атакует врагов и регенерирует энергию Гань Юй. Поэтому следует стараться попасть по противнику и установкой, и взрывом от уничтожения.<br>


# === characters.#ganyu.roles.#off-field_dps.notes ===

Как карманный ДД Гань Юй отлично играет в отрядах Заморозки, где у неё 100% аптайм Взрыва стихии и её Взрыв стихии баффает Крио урон отряда.

**О выборе оружия**

[Прототип: Полумесяц](#weapon:prototype-crescent)<br>
Этот оружие может превзойти некоторые другие варианты, если активировать его бафф перед использованием Взрыва стихии.

[Элегия погибели](#weapon:elegy-for-the-end)<br>
Даёт баффы на Мастерство стихий и Силу атаки, полезные для отряда играющего через реакции.

**О комплектах артефактов**

[Заблудший в метели (4)](#artifact:blizzard-strayer)<br>
Даёт мощный бонус для команды Заморозки.

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Более универсальный сет, так как даёт 20% бонус Силы атаки всему отряду.


# === characters.#gorou.roles.#support.name ===

Саппорт


# === characters.#gorou.roles.#support.tips ===

\[С2\] увеличивает длительность Взрыва стихии Горо c 9 до 12 секунд при получении щита от Кристаллизации. После Взрыва стихии рекомендуется переключаться на ДД, чтобы они максимально использовали время баффа.


# === characters.#gorou.roles.#support.notes ===

Горо — это герой поддержки, баффы которого зависят от количества Гео персонажей в отряде. Если Гео персонаж в отряде один, Элементальный навык Горо даст персонажу, находящемуся внутри радиуса его действия, бафф Защиты. Если два — сопротивление прерыванию. Если три — бонус Гео урона. Его Взрыв стихии создает область, двигающуюся вместе с активным персонажем и дающую те же баффы, что Элементальный навык, а так же наносящую периодический урон. Так как Горо специализируется на баффе Защиты и Гео урона, он специализированный саппорт под персонажей, которые скейлятся от Защиты — например, Ноэль или Итто. Он может быть нишевым саппортом для других Гео персонажей на \[С6\].

**О выборе оружия**

[Элегия погибели](#weapon:elegy-for-the-end)<br>
Элегия полезна своим допстатом на Восстановление энергии, а также баффом Мастерства стихий и процента Силы атаки для отряда. Хотя это не особо полезно для комманд построенных вокруг Ноэлли или Итто, Элегия может быть полезна другим отрядам.

**О комплектах артефактов**

[Изгнанник (4)](#artifact:the-exile)<br>
Использование Взрыва стихии, восстанавливает 6 единиц энергии каждому в отряде. Но стоит обратить внимание, что у этого набора нет артефактов легендарного уровня, а значит максимальный уровень прокачки ограничен 16. Это приведет к потере 34% от главстата артефакта и 25% от допстста, если сравнивать с легендарным артефактом 20 уровня.

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Полезный комплект, когда Горо собрал достаточно Восстановления энергии и не нуждается в комплектах артефактов с Восстановлением энергии.

**О приоритетных главстатах и допстатах в артефактах**

Защита в главстатах и допстатах и, возможно, корона на Бонус лечения, могут быть собраны на \[С4\] из-за возможности лечения активного персонажа Взрывом стихий, в случае если Горо единственный лекарь в отряде. Лечение скейлится от Защиты.

Корона и допстаты на Шанс крит. попадания могут собираться когда Горо использует Боевой лук Фавония, для частой активировации бонуса лука.

Допстаты на Восстановление энергии могут быть важнее, чем правильный главстат, так как влияние урона Горо в отряде незначительное. Гораздо ценнее получать энергию для его Взрыва стихии.


# === characters.#hu-tao.roles.#dps.name ===

ДД


# === characters.#hu-tao.roles.#dps.tips ===

Если заканчивать серии атак Ху Тао Взрывом стихии и не отменить анимацию последней заряженной атаки прыжком или бегом, её Взрыв стихии получит бонус от Элементального навыка Ху Тао.

**Нужно отменять анимацию атаки для максимизации урона.** Последовательность действий следующая:<br>
**О2ЗП** на **\[C0\]** и **О2ЗС** for **\[С1\].**<br>
**О3ЗП/О3ЗП** чтобы пожертвовать выносливостью и нанести больший урон.

**\*О = обычная атака, З = Заряженная атака, П = Прыжок, С = Спринт**


# === characters.#hu-tao.roles.#dps.notes ===

Ху Тао наиболее эффективна в команде с Син Цю, где она может вызывать реакции Пара или Таяния замороженных и использовать свой топовый комплект артефактов, Горящую алую ведьму (4). Игра через Таяние замороженных подразумевает атаки по замороженным врагам, потому что в отличие от Дилюка или Кли, Ху Тао не разбивает заморозку.

**О выборе оружия**

[Нефритовый коршун](#weapon:primordial-jade-winged-spear)<br>
Это оружие очень сильно, даже сильнее \[Р1\]-версий [Грозы драконов](#weapon:dragons-bane) и [Смертельного боя](#weapon:deathmatch), но только когда собраны все стаки баффа. А сделать это на Ху Тао довольно сложно, что делает это оружие менее надежным, чем остальные.

[Смертельный бой](#weapon:deathmatch)<br>
Смертельный бой лучше, чем Гроза драконов, когда Ху Тао не вызывает Пар.

[Черногорская пика](#weapon:blackcliff-pole)<br>
Не смотря на кажущуюся полезность из-за допстата на Крит. урон, Черногорская пика с 1 стаком лишь слегка лучше, чем **\[Р5\]** Белая кисть, а поддерживать  стаки все время — сложно. Не рекомендуется покупать это оружие специально для Ху Тао. Стоит использовать только если оно уже куплено.

[Покоритель вихря](#weapon:vortex-vanquisher) и [Небесная ось](#weapon:skyward-spine)<br>
Почти ничего не дают Ху Тао, так как её урон растет больше от объема её ХП, чем от процента Силы атаки.

[Белая кисть](#weapon:white-tassel)<br>
Белая кисть заставляет чаще бить обычной атакой, чтобы получить бафф от бонуса оружия. Обычно Ху Тао играется через чередование одной обычной атак и одной заряженной, в случае Белой Кисти рекомендуется чередовать две обычные атаки с одной заряженной.

**О комплектах артефактов**

[Воспоминания Симэнавы (4)](#artifact:shimenawas-reminiscence)<br>
Дает больший урон с обычных атак, чем полный комплект Горящей Алой ведьмы, но понижает урон Взрыва стихии, что портит урон Ху Тао по площади. Так же, без возможности ультовать, у Ху Тао могут возникнуть проблемы с выживанием, если в команте отсутствует целитель.

[Встречная комета (4)](#artifact:retracing-bolide) **и** [Странствующий ансамбль ](#artifact:wanderers-troupe)**или**[ Стойкость Миллелита (2) ](#artifact:tenacity-of-the-millelith)**с**[ Горящей алой ведьмой (2)](#artifact:crimson-witch-of-flames)<br>
Все эти комплекты равны по суммарному урону, при учете того, что Встречная комета (4) используется  при всегда активном щите. Разница лишь в том, что с Кометой выше урон обычных атак и ниже урон Взрыва стихий, а так же необходимо всегда поддерживать щит.

**О приоритетных допстатах в артефактах:**

Так как Ху Тао может вызывать Пар на своих самых сильных умениях, Мастерство стихий довольно-таки ценно на ней. В случае, если у Ху Тао меньше 100 Мастерства стихий, включая бафф Сахарозы и прочие, допстаты на Мастерство стихий важнее Шанса крит. попадания или Крит. урона.


# === characters.#itto.roles.#dps.name ===

ДД


# === characters.#itto.roles.#dps.notes ===

Итто — это Гео ДД, наносящий большой урон после активации Взрыва стихии, который даёт ему бонус Силы атаки и Гео инфузию.

Итто накапливает уровни Невероятной силы когда вторая и четвертая обычная атака попадает по врагу, а во время Взрыва стихии, добавляется ещё и первая и третья атаки. Накопив максимум стаков Невероятной силы, пять штук, он может использовать усиленную заряженную атаку, называющуюся Аратаки кэсагири.

Элементальный навык Итто призывает бычка Уси, который отвлекает врагов, считается Гео конструкцией, наносит урон противникам во время активации и даёт один стак Невероятной силы. Когда Уси покидает поле боя, то даёт еще один стак Невероятной силы. При получении урона Уси также даёт один стак Невероятной силы, но не чаще одного раза в две секунды.

Взрыв стихии наделяет обычные и Заряженные атаки Итто Гео инфузией, увеличивает скорость его обычных атак и увеличивает Силу атаки на определенный процент от текущей защиты Итто. Значение защиты снапшотается.

**О выборе оружия**

[Меч драконьей кости](#weapon:serpent-spine)<br>
С комплектом [Кокон сладких грёз](#artifact:husk-of-opulent-dreams), стаки бонуса от Меча драконьей кости важнее стаков Кокона.

[Небесное величие](#weapon:skyward-pride)<br>
Может быть отличным или плохим оружием для Итто в зависимости от потребности Восстановления энергии. Чем меньше необходимость Восстановления энергии, что зависит от генерации частиц членов отряда или Восстановления энергии, накопленного в допстатах, тем менее полезным становится это оружие.

[Белая тень](#weapon:whiteblind)<br>
Так как Взрыв стихии снапшотает Защиту, то после его активации новые стаки баффа оружия не повлияют на урон Взрыва стихии.

**О комплектах артефактов**

[Встречная комета (4)](#artifact:retracing-bolide)<br>
Встречная комета дает дополнительный 40% бонус урона обычной и Заряженной атаки, если Итто находится в щите. Щит от Кристаллизации не надёжен, и лучше использовать хорошего щитовика, например, Чжуна Ли.


# === characters.#jean.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#jean.roles.#burst_support.notes ===

Элементальный навык Джинн подбрасывает в воздух маленьких противников, которые потом получают урон от падения, и прерывает атаку больших. Её лечение скейлится от Силы атаки.

**О выборе оружия**

[Амэнома Кагэути](#weapon:amenoma-kageuchi)<br>
Это оружие может понизить потребность Восстановления энергии до 150% или даже 0%, в зависимости от количества стаков пассивки и Ранга пробуждения этого оружия. На более высоком Ранге пробуждения, с большим числом стаков пассивки, это оружие лучше, чем [Небесный меч](#weapon:skyward-blade), а на более низком ранге, хуже.

[Клятва свободы](#weapon:freedom-sworn)<br>
Позиция в рейтинге учитывает только собственный урон Джинн, без учета баффа отряда. Если нужно баффать глав ДД, то можно выдать этот меч Джинн, потому что она легко может активировать бонус этого оружия.

[Драконий рык](#weapon:lions-roar)<br>
Этот меч может быть лучше [Меча Фавония](#weapon:favonius-sword) при 50% времени действия бонуса оружия, а при 100% времени действия бонуса даже лучше [Меча Сокола](#weapon:aquila-favonia).

**О комплектах артефактов**

[Изумрудная тень (4)](#artifact:viridescent-venerer)<br>
Топовый комплект для любых Анемо персонажей, так как срезает 40% элементальной Защиты противнику.

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Этот комплект даёт 20% бонус Силы атаки всему отряду.

[Моллюск морских красок (4)](#artifact:ocean-hued-clam)<br>
Лучший комплект для личного урона Джинн. Но её личный урон всё равно останется маленьким. Гораздо полезнее будет носить на Джинн комплект, дающий бонусы всему отряду, например Церемонию древней знати или Изумрудную тень.

**О выборе главстатов**

Делать упор на Мастерство стихий стоит только в паре с Беннетом, так как Взрыв стихии Джинн поглотит Пиро от Взрыва Беннетта и начнёт поджигать и наносить Пиро урон. Поэтому предпочтительны оружия с бонусом Мастерства стихии и Восстановления энергии ([Клятва свободы](#weapon:freedom-sworn), [Небесный меч](#weapon:skyward-blade), [Меч Фавония](#weapon:favonius-sword), [Церемониальный меч](#weapon:sacrificial-sword), [Стальное жало](#weapon:iron-sting), [Амэнома Кагэути](#weapon:amenoma-kageuchi)).

**О приоритетных талантах**

Если важнее урон, чем лечение, то в первую очередь стоит качать Элементальный навык, потому что его урон значительнее растет от прокачки, чем урон Взрыва стихии.

Если Джинн единственный Анемо персонаж в отряде, то перед ней стоит выбор: вкладываться в Силу атаки для более сильного лечения, но редко мочь активировать Взрыв стихии, или наоборот, вкладываться в Восстановление энергии, с более слабым лечением, но частым Взрывом стихии.

Если Джинн не единственный Анемо в отряде, то у нее нет проблемы с Восстановлением энергии, и она может полностью вкладываться в урон.


# === characters.#kaeya.credits ===

[Подробный разбор](https://keqingmains.com/kaeya/) от Кэйа мейнеров и ККМ (англ.)


# === characters.#kaeya.roles.#cryo_dps.name ===

Крио ДД


# === characters.#kaeya.roles.#cryo_dps.subStats.notes ===

**шанс крит. попадания:** обычно не так полезен, как другие боевые статы, благодаря бонусам [Заблудшего в метели](#artifact:blizzard-strayer) и Крио резонанса


# === characters.#kaeya.roles.#cryo_dps.talents.notes ===

Элементальный навык важнее обычной атаки, если в команде нет Чун Юня


# === characters.#kaeya.roles.#cryo_dps.tips ===

**\[С1\]** против врагов со статусом Крио


# === characters.#kaeya.roles.#cryo_dps.notes ===

Крио ДД Кэйа обычно используется в отряде с Чун Юнем и Син Цю или Барбарой, и носит комплект Заблудшего в метели (4). В таком отряде у Кэйи появляется 55% Шанса крита против Замороженных врагов еще до сбора дополнительных Крит. статов, поэтому вместо них можно собирать Силу атаки или Крит. урон.

**О комплектах артефактов**

[Бледный огонь (4)](#artifact:pale-flame)<br>
Бонус этого комплекта сложно полностью получить Кэйей, потому что время отката его Элементального навыка 6 секунд. Для полного эффекта комплекта надо набить два стака, а с таким временем отката это тяжело, особенно если поддерживать ротацию между персонажами.


# === characters.#kaeya.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#kaeya.roles.#burst_support.notes ===

Взрыв стихии Кэйи — отличный способ накладывать Крио находясь рядом с противником. Из-за особенностей нанесения Крио урона, его Взрыв стихии не может использоваться в отрядах Реверсивного Таяния.

**О выборе оружия**

[Клятва свободы](#weapon:freedom-sworn)<br>
Это оружие подходит только для отрядов Реверсивного Таяния, в которых ценится Мастерство стихий.

[Небесный меч](#weapon:skyward-blade)<br>
В отряде со вторым Крио, бонус Восстановления энергии этого оружия будет не нужен, и меч будет менее полезен.

[Драконий рык ](#weapon:lions-roar)**/**[ Вспышка во тьме ](#weapon:the-alley-flash)**/**[ Стальное жало](#weapon:iron-sting)<br>
Это оружие расположено в рейтинге с учетом использования их в отрядах Реверсивного Таяния, в которых ценится Мастерство стихий. На \[Р5\], эти оружия превзойдут Кромсателя пиков.

**О комплектах артефактов**

[Инструктор (4)](#artifact:instructor)<br>
Из-за возможности часто активировать пассивный бонус Инструктора, этот комплект хорошо подходит для Кэйи в отрядах Реверсивного Таяния. Кроме Восстановления энергии, остальные статы не так важны, что делает этот комплект хорошим бюджетным вариантом.

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Рекомендуется только в том случае, если никто другой в отряде не носит этот комплект.


# === characters.#kazuha.credits ===

[Подробный разбор](https://keqingmains.com/kazuha/) от ККМ (англ.)


# === characters.#kazuha.roles.#em_build.name ===

Сборка через МС


# === characters.#kazuha.roles.#em_build.tips ===

Элемент который поглотит Элементальный взрыв Кадзухи находится в таком приоритете: Пиро > Гидро > Электро > Крио, и способен поглотить как с активного персонажа так и с врагов. Например, нахождение в поле, созданном Взрывом стихии Беннета, наделит Взрыв стихии Кадзухи Пиро инфузией, так как Беннет накладывает эффект Пиро на активного персонажа.


# === characters.#kazuha.roles.#em_build.notes ===

Вызывая реакцию Рассеивания, благодаря пассивному таланту 60 уровня, Кадзуха баффает соответствующий Элементальный урон всех членов отряда. Сила баффа скейлится от Мастерства стихий Кадзухи. Поэтому нужно собирать максимально возможное количество Мастерства стихий.

Также, как и у Венти, на всех частях артефактов нужно собирать Мастерство стихий в главстатах и сабстатах, для увеличения наносимого урона. Стоит собирать атаку-урон-криты только в случае Гео отрядов, моно Анемо отрядов или в случае битвы с один противником, а не группой. Единственные ситуации, когда в артефактах собирается атака-урон-криты, это Гео отряды, отряды только из Анемо персонажей или бой с одним противником, а не группой.

**О выборе оружия**

Главной целью является сбор как можно большего количества Мастерства стихий при этом имея достаточно Восстановления энергии, 160–180%, для постоянного использования Взрыва стихии. Наличие [Церемониального меча](#weapon:sacrificial-sword) или второго Анемо персонажа в отряде также поможет уменьшить потребность в Восстановлении энергии и набрать больше допстатов на Мастерство стихий.

[Стальное жало](#weapon:iron-sting)<br>
Стальное жало даёт наивысшее количество Мастерства стихий среди других четырехзвездочных мечей, а также бафф всего отряда. Если у Кадзухи нет пробем с Восстановлением энергии, то это оружие лучше, чем любые другие четырехзвездочные мечи на Восстановление энергии.

**О комплектах артефактов**

[Громогласный рёв ярости (4)](#artifact:thundering-fury)<br>
Этот нишевый билд, который подходит только для битв с группами врагов и требует особых отрядов. В таких отрядах должны быть карманные Гидро и Электро саппорты, чтобы Кадзуха все время активировал бонус Громогласного рёва ярости (4) своим Элементальным навыком, а также второй Анемо саппорт с [Изумрудной тенью (4)](#artifact:viridescent-venerer). Пример такого отряда: Кадзуха, Кокоми, Фишль и Венти, или с Барбарой и Сахарозой как более слабой заменой Кокоми и Венти. Такой отряд может дополнительно наносить врагам урон от падения, а Фишль с [Зеленым луком](#weapon:the-viridescent-hunt), позволяет наносить урон от падения еще чаще. Следует обратить внимание, что этот отряд очень слабо работает в бою против одного противника.


# === characters.#keqing.credits ===

[Подробный разбор](https://keqingmains.com/keqing/) от ККМ (англ.)


# === characters.#keqing.roles.#physical_dps.name ===

Физ. ДД


# === characters.#keqing.roles.#physical_dps.talents.notes ===

См. примечания ниже


# === characters.#keqing.roles.#physical_dps.notes ===

Этот раздел посвящен игре на Кэ Цин без накладывания Электро на атаки, и сфокусирован на игре через физический урон. Для постоянной игры через физический урон нужно делать заряженную атаку после использования Элементального навыка, а не использовать навык повторно, то есть не телепортироваться к стилету Кэ Цин.

**О комплектах артефактов**

**[Бледный огонь (4)](#artifact:pale-flame)**<br>
Лучший комплект на Кэ Цин, потому что она всегда может использовать бонус от комплекта, делая заряженную атаку после Элементального навыка.

**[Бледный огонь (2) ](#artifact:pale-flame)+ [Рыцарь крови (2)](#artifact:bloodstained-chivalry)**<br>
Набор, который можно использовать только до получения Бледного огня (4).

**[Рыцарь крови (2) ](#artifact:bloodstained-chivalry)+ [18% Сила атаки (2)](#artifact:18%-atk)**<br>
Стандартный набор для физ. ДД.

**[Встречная комета (4)](#artifact:retracing-bolide)**<br>
Этот комплект используется только под постоянным щитом, например, с Чжуном Ли или Дионой.

**[Рыцарь крови (4)](#artifact:bloodstained-chivalry)**<br>
Из-за своего баффа, этот комплект подходит только для битвы с противникам, которые убиваются за 10 секунд.


# === characters.#keqing.roles.#electro_dps.name ===

Электро ДД


# === characters.#keqing.roles.#electro_dps.artifacts.notes ===

Громогласный рёв ярости (4) не рекомендуется для Электро ДД Кэ Цин.

Обратите внимание, что артефакты отсортированы не по рейтингу. Подробности в описании.


# === characters.#keqing.roles.#electro_dps.talents.notes ===

Кэ Цин получает 15% бонусного шанса Крит. попадания после использования своего Взрыва стихии, поэтому его и Обычную атаку стоит прокачивать с одинаковым приоритетом. Но и про Элементальный навык забывать не нужно.


# === characters.#keqing.roles.#electro_dps.tips ===

**Оптимальная последовательность атак**

Элементальный навык (бросить стилет, но не активировать)-> Взрыв стихии -> Элементальный навык (активировать стилет) -> Спам атаками-> Повторить

**О ролях**

И физ, и Электро Кэ Цин хороши. Стиль игры обычно выбирается в зависимости от того, какое оружие повезло выролить. Например, Драконий рык — Электро билд, Меч-флейта одинаково хорош всегда, и т.д.


# === characters.#keqing.roles.#electro_dps.notes ===

**О выборе оружия**

**[Кромсатель пиков](#weapon:summit-shaper)**<br>
Это оружие на \[Р2\] лучше, чем Драгоценный омут \[Р1\], если персонаж под щитом.

**[Чёрный меч](#weapon:the-black-sword)**<br>
На Кэ Цин, играющей только через умения, это оружие хуже, чем Предвестник зари \[Р5\].

**О комплектах артефактов**

**[Громогласный рёв ярости (2) ](#artifact:thundering-fury)+ [18% Силы атаки (2)](#artifact:18%-atk)**<br>
Универсальный комплект на все случаи.

**[Усмиряющий гром (4)](#artifact:thundersoother)**<br>
Этот набор нужно использовать в отрядах через реакцию Заряжен, например в команде с Син Цю, или в моно Электро отрядах, хотя это не рекомендуется. Этот комплект увеличивает урон на 35%, но для этого бонуса нужно всегда поддерживать Электро на врагах.

**[Громогласный рёв ярости (2) ](#artifact:thundering-fury)+ [Церемония древней знати (2)](#artifact:noblesse-oblige)**<br>
Этот комплект используется на Кэ Цин, играющей через Взрыв стихии или только умения. Взрыв стихий — основной источник урона с этим набором.


# === characters.#klee.credits ===

[Подробный разбор](https://keqingmains.com/klee/) от Кли мейнеров и ККМ (англ.)


# === characters.#klee.roles.#dps.name ===

ДД


# === characters.#klee.roles.#dps.tips ===

Атаки Кли имеют 50% шанс создать взрывную искру. С ней следующая заряженная атака не будет потреблять выносливость и увеличивает урон этой атаки на 50%.


# === characters.#klee.roles.#dps.notes ===

Будучи хрупкой, Кли нуждается в правильном позиционировании. Этого трудно достигнуть из-за её маленького роста. Могут помочь постоянные прыжки во время Заряженных атак.

**О выборе оружия**

[Молитва святым ветрам](#weapon:lost-prayer-to-the-sacred-winds)<br>
Бафф Беннета или бафф Эпоса о драконоборцах плюс комплект Церемонии древней знати (4) выдвигает Молитву святым ветрам вперед Небесного атласа и Памяти о пыли.

[Истории Додоко](#weapon:dodoco-tales)<br>
На одинаковом ранге пробуждения, Песнь странника лучше, чем Истории Додоко, но сравния \[Р5\] Истории Додоко и \[Р1-3\] Песнь странника, \[Р5\] Истории Додоко выигрывают. Обратите внимание, что бафф Беннета или Эпоса о драконоборцах плюс бафф комплекта Церемонии древней знати (4) может вывести Песнь странника и Солнечную жемчужину вперед Историй Додоко, даже при низком Ранге пробуждения.

**О комплектах артефактов**

[Ступающий по лаве (4)](#artifact:lavawalker)<br>
Особенности игры на Кли делают вызов реакций Пара и Таяния сложнее, чем у других Пиро ДД, но её постоянное наложение Пиро позволяет ей использовать комплект Ступающего по лаве. [Табличка с расчетами на английском](https://docs.google.com/spreadsheets/d/1JJZPMoNOqXhX8Ez1cl63fNXESJZXx7kQaJEeQ-WLOEY/edit#gid=186983353)

[Горящая алая ведьма (4)](#artifact:crimson-witch-of-flames)<br>
Этот комплект лучше всего подходидит для Кли играющей через Пар, но стоит обратить внимание, что из-за быстрого наложения Пиро, она не сможет вызывать Пар на большинстве своих обычных или Заряженных атак.

[Странствующий ансамбль (4)](#artifact:wanderers-troupe)<br>
Странствующий ансамбль увеличивает урон Заряженной атаки на 35%. Но реализовать этот бонус будет сложно, потому что игра через Заряженные атаки заставит чаще уворачиваться и тратить больше выносливости.


# === characters.#kokomi.credits ===

[Подробный разбор](https://keqingmains.com/kokomi/) от Кокоми мейнеров и ККМ (англ.)


# === characters.#kokomi.roles.#support.name ===

Саппорт


# === characters.#kokomi.roles.#support.mainStats.notes ===

Если дополнительное лечение не требуется, главстаты для [Стойкости Миллелита (4)](#artifact:tenacity-of-the-millelith) неважны. Лучше сфокусироваться на главстатах Восстановления энергии.


# === characters.#kokomi.roles.#support.tips ===

Взрыв стихии Кокоми увеличивает урон её Элементального навыка, но только при условии личного пребывания Кокоми на поле боя.


# === characters.#kokomi.roles.#support.notes ===

Саппорт Кокоми использует Взрыв стихии только для того, чтобы продлить время действия её Элементального навыка. Это позволяет всё время держать медузу на поле боя и стабильно накладывать Гидро на противников.

**О выборе оружия**

[Кольцо Хакусин](#weapon:hakushin-ring)<br>
Это оружие увеличивает урон отрядов, играющих через Заряжен, но с ним Кокоми хуже лечит.

[Церемониальные мемуары](#weapon:sacrificial-fragments)<br>
Может быть полезным, чтобы постоянно держать медузу на поле боя без использования Взрыва стихии.

**О комплектах артефактов**

[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)<br>
Правильно используя Элементальный навык, Кокоми может держать бонус комплекта всегда активированным.

[Моллюск морских красок (4)](#artifact:ocean-hued-clam)<br>
Если Кокоми атакует противников во время своего Взрыва стихии, этот комплект лучше Стойкости Миллелита (4). В случае же использования Кокоми только для Элементального навыка, то этот комплект хуже Стойкости Миллелита (4).

**Об основных характеристиках и допстатах**

**Восстановление энергии**<br>
У Кокоми должно быть такое Восстановление энергии, чтобы она всегда могла использовать Взрыв стихий перед Элементальным навыком, для беспрерывного наличия медузы на поле боя. Без Прототипа: Янтаря, Кокоми потребуется \~200-220%+ Восстановления энергии. С Янтарем на \[Р1\] — 170-190% и 150-170% на \[Р5\]


# === characters.#kokomi.roles.#dps.name ===

ДД


# === characters.#kokomi.roles.#dps.tips ===

Из-за высокого потребления выносливости её заряженными атаками, оптимальная последовательность атак такая:

На \[C0\]: 2ОА С<br>
На \[С1\]: 3ОА С

\*ОА — обычная атака, С — спринт

При игре с Бэу Доу последовательность 3ОА С может привести к потере урона из-за особенности её Взрыва стихии. Поэтому эта последовательность рекомендуется только при игре через Пар.


# === characters.#kokomi.roles.#dps.notes ===

ДД Кокоми строится через урон во время её Взрыва стихий, потому что вне Взрыва стихий её урон маленький.

**О выборе оружия**

**[Эпос о драконоборцах](#weapon:thrilling-tales-of-dragon-slayers)**<br>
Хотя это оружие и не так сильно по урону, его бафф отряда весьма полезен. Это оружие может быть лучшим для суммарного урона отряда, если в нем больше никто не носит Эпос о драконоборцах

**[Кольцо Хакусин](#weapon:hakushin-ring)**<br>
Это оружие лучший выбор для отрядов через Заряжен.

**О комплектах артефактов**

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)**<br>
Хотя этот комплект хуже всего по личному урону Кокоми, бонус урона отряду иногда может быть более полезен, чем ее личный урон.


# === characters.#kujou-sara.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#kujou-sara.roles.#burst_support.subStats.notes ===

В первую очередь, стоит прокачивать Восстановление энергии до тех пор, пока не получится ультовать по откату.


# === characters.#kujou-sara.roles.#burst_support.tips ===

Чтобы получить частицы от ее Элементального навыка, нужно чтобы взорвавшееся после заряженной атаки Воронье перо задело противника. Тогда навык сгенерирует три частицы.

Одно только нажатие Элементального навыка не генерирует частицы, на \[С2\] в том числе.


# === characters.#kujou-sara.roles.#burst_support.notes ===

Сара — героиня поддержки, усиляющая отряд механиками Вороньего пера. Когда Сара использует свой Элементальный навык, её следующий заряженный выстрел оставит Воронье перо в месте попадания, затем Воронье перо взорвется и персонаж в области взрыва получит бонус на силу атаки, зависящий от базовой силы атаки Сары.

На \[С2\], Элементальный навык сразу накладывает бафф Вороньего пера, при этом остается возможность баффнуться Пером с заряженной атаки. Для максимальной продолжительности действия баффа, следует сначала наложить бафф простым нажатием Элементального навыка, и, через некоторое время, использовать бафф от заряженной атаки. Так же появляется возможность использовать Взрыв стихии после Элементального навыка, чтобы он получил от него бонус урона, а затем Пером с заряженной атакой баффнуть другого персонажа. Стоит отметить, что бонус Электро Крит. урона на \[С6\] не снапшотается и не отображается в профиле персонажа.

**О выборе оружия**

Оружие отсортировано по **Базовой атаке,** чтобы максимизировать бафф Сары.

**[Элегия погибели](#weapon:elegy-for-the-end)**<br>
Время отката бонуса этого оружия идеально совпадает со временем отката Взрыва стихий Сары. У этого оружия ниже базовая атака по сравнению с Небесным крыло, но Элегия лучше, потому что Сара может все время использовать бонус этого оружия и давать отряду дополнительный бафф на 100 Мастерство стихий и 20 процентов Силы атаки на 12 секунд, кроме того, у Элегии допстат на Восстановление энергии, что важно для Сары играющей через Взрыв стихии.

**О комплектах артефактов**

**[Церемония древней знати (4)](#artifact:noblesse-oblige)**<br>
Универсальный комплект чтобы баффать Силу атаки отряда на 20%, можно использовать, если в отряде больше никого нет с этим комплектом


# === characters.#lisa.credits ===

[Подробный разбор](https://keqingmains.com/lisa/)  от  Лиза мейнеров и ККМ (англ.)


# === characters.#lisa.roles.#electro_dps.name ===

Электро ДД


# === characters.#lisa.roles.#electro_dps.tips ===

**Оптимальная последовательность атак Лизы**

ЭН (быстрое нажатие) > ОА x2 \[до 3 стаков\] > Полностью заряженный ЭН (долгое нажатие) > ОА х2 > ЗА. Повторять \[ОА х2 > ЗА\] пока ЭН не откатится, затем повторить сначала.

**ОА = обычная атака, ЗА = Заряженая атака, ЭН = Элементальный навык**


# === characters.#lisa.roles.#electro_dps.notes ===

**О комплектах артефактов**

**[Усмиряющий гром (4)](#artifact:thundersoother)**<br>
Для отрядов фокусирующихся на реакциях Заряжен, **Усмиряющий гром (4)** может превзойти по урону **Громогласный рёв ярости (2)  плюс комплект на 18 процентов Силы атаки (2).** Этот комплект ниже в рейтинге из-за меньшей универсальности.

Электро ДД Лиза хорошо играется через отряды, построенные вокруг реакции Заряжен. Для отрядов с игрой через Перегрузку, оптимальнее собирать билд ДД через Мастерство стихий.

**Regarding Weapons**

**[Истина кагура](#weapon:kaguras-verity)**<br>
Позиция в рейтинге основана на поддержании третьего уровня бонуса оружия, для чего нужно исполнять такую последовательность атак:

БЭН > 2 ОА> БЭН > 2 ОА > БЭН > ВС > 1 ОА > ДЭН > 1 ОА > 1 ЗА > 3 ОА> 2 ОА> 1 ЗА

Где БЭН — быстрое нажатие Элементального навыка, ОА — обычная атака, ДЭН — долгое нажатие Элементального навыка, ВС — Взрыв стихии, ЗА — заряженная атака.

[Подробный расчёт (англ.)](https://docs.google.com/spreadsheets/d/1kBaIxbNUUkShl-OwbOxMK4wyPQvL-WWeOko7yfWRIxw/edit#gid=0).


# === characters.#lisa.roles.#em_dps.name ===

Сборка через МС


# === characters.#lisa.roles.#em_dps.talents.notes ===

для Лизы в сборке через Мастерство стихий уровни талантов не очень важны


# === characters.#lisa.roles.#em_dps.notes ===

ДД Лиза через Мастерство стихий нацелена быть постоянным активатором для реакций, и реакция Перегрузка больше всего подходит для этой сборки. Следует избегать второго Электро персонажа в отряде, чтобы он не сбивал Пиро с противников.

Отряды через реакцию Заряжен плохо играются через Мастерство стихий, поэтому для игры через эту реакцию следует собрать Лизу в Электро ДД или карманного ДД.

**О приоритете характеристик**

Следует инвестировать как в Мастерство стихий, так и в Восстановление энергии. Цель — собрать достаточное количество Восстановления энергии, точнее, 170-190 и при этом иметь большое количество Мастерства стихий.


# === characters.#lisa.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#lisa.roles.#off-field_dps.tips ===

Не обязательно долго ждать пока Лиза сама ударит Элементальным навыком. Долгое нажатие Элементального навыка Лизы требует менее четырех секунд для полной зарядки. Как только индикатор сравняется с кольцом, умение уже полностью заряжено и можно отпустить клавишу.


# === characters.#lisa.roles.#off-field_dps.notes ===

Может быть собрана для урона со Взрыва стихии или Элементального навыка.<br>
Лиза хорошо сочетается с Анемо персонажами, которые помогают удерживать противников внутри радиуса её Взрыва стихии.

**О выборе оружия**

**[Истина кагура](#weapon:kaguras-verity)**<br>
Позиция в рейтинге основана на поддержании третьего уровня бонуса оружия, для чего нужно исполнять такую последовательность атак:

БЭН > 2 ОА > БЭН > ВС > ДЭН

Где БЭН — быстрое нажатие Элементального навыка, ОА — обычная атака, ДЭН — долгое нажатие Элементального навыка, ВС — Взрыв стихии.

**[Эпос о драконоборцах](#weapon:thrilling-tales-of-dragon-slayers)**<br>
Этот вариант **рекомендуется только для Лизы в дешевой сборке, где она фокусируется на баффе команды.**

**О приоритете характеристик**

Нужно добиваться максимального аптайма Взрыва стихии Лизы.


# === characters.#mona.roles.#dps.name ===

ДД


# === characters.#mona.roles.#dps.tips ===

<br>


# === characters.#mona.roles.#dps.notes ===

Этот билд лучше работает на \[С2\]. Некоторые четырех звездочные персонажи могут выдавать больший урон, чем ДД Мона, поэтому Мона лучше как доп. ДД или героиня поддержки.<br>


# === characters.#mona.roles.#nuke.name ===

Ульт-саппорт


# === characters.#mona.roles.#nuke.tips ===

Чтобы Взрыв стихии Моны нанес удвоенный урон через реакцию Пар, нужно действовать в такой последовательности:<br>
1. Наложить на врага Пиро<br>
2. Использовать Взрыв стихии Моны, что должно снять статус Пиро с врага<br>
3. Подождать две с половиной секунды, чтобы прошел внутренний таймер Моны. Если в отряде есть Беннет, Мона должна оставаться активной, чтобы продолжалось действие ее баффа<br>
4. Переключиться на Пиро персонажа и ударить врага Пиро атакой<br>
5. Взрыв стихии Моны взорвется, наложит Гидро и вызовет Пар


# === characters.#mona.roles.#nuke.notes ===

Элементальный навык Моны отвлекает врагов и накладывает на них Гидро, её Взрыв стихии наносит огромный урон и навешивает дебафф на противников, а также лишает маленьких врагов возможности двигаться, если разница уровня Моны и врага меньше 20.<br>


# === characters.#mona.roles.#freeze_support.name ===

Саппорт для Заморозки


# === characters.#mona.roles.#freeze_support.mainStats.notes ===

Атакующие статы Моне для этой роли не нужны, лучше сфокусироваться на бонусе комплекта артефактов


# === characters.#mona.roles.#freeze_support.subStats.notes ===

Атакующие статы не нужны, лучше собрать достаточно Восстановления энергии для своевременного накопления Взрыва стихии.


# === characters.#mona.roles.#freeze_support.tips ===

Взрыв стихии Моны накладывает пузырь и Омен во время каста, но откат умения начинается когда был взорван пузырь.

Пузырь держится на замороженных врагах пока не спадет Заморозка. Эта особенность делает Мону приоритетной героиней поддержки для отрядов через Заморозку.


# === characters.#mona.roles.#freeze_support.notes ===

Основная роль Моны в отряде через Заморозку — накладывать на врагов Омен, так как время действия Омена будет увеличено из-за особенности статуса Заморозки. Стоит обратить внимание, что перед использованием её Взрыва стихий нужно наложить на противника Крио. Замороженное состояние может быть продлено, если продолжать накладывать на противника Крио и Гидро.

**Regarding Artifact Sets**<br>

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)**<br>
Сет подойдёт, если в команде уже есть персонаж с [Церемонией древней знати (4)](#artifact:noblesse-oblige).

**О выборе оружия**

[Эпос о драконоборцах](#weapon:thrilling-tales-of-dragon-slayers)<br>
Лучший выбор для Моны в отряде Заморозки, потому что он дополнительно увеличивает силу атаки глав. ДД, но только при условии, что Мона использует Взрыв стихии по откату. В противном случае, следует взять оружие на Восстановление энергии.


# === characters.#ningguang.roles.#dps.name ===

ДД


# === characters.#ningguang.roles.#dps.tips ===

Элементальный навык Нин Гуан генерирует энергию каждые 6 секунд, поэтому чтобы чаще использовать Взрыв стихии, следует ждать 6 секунд между использованиями Элементального навыка. Ширма ломается при установке поверх другой Гео структуры.


# === characters.#ningguang.roles.#dps.notes ===

У Нин Гуан низкая базовая атака и нет никаких других собственных источников Силы атаки, поэтому рекомендуется использовать её с Беннетом, Пиро резонансом или баффать её силу атаки как-то еще.

**О выборе оружия**

Рейтинг сформирован с учетом наличия внешнего баффа Силы атаки Нин Гуан.

[Молитва святым ветрам](#weapon:lost-prayer-to-the-sacred-winds)<br>
Хорошее оружие при наличии внешнего баффа силы атаки, из-за своего стата на Шанс крит. попадания.

[Память о пыли](#weapon:memory-of-dust)<br>
Лучшее оружие, когда у Нин Гуан нет никакого внешнего баффа на Силу атаки.

[Песнь странника](#weapon:the-widsith)<br>
Хорошее оружие с полезными бонусам, но бонус Мастерства стихий бесполезен для Нин Гуан.

[Истории Додоко](#weapon:dodoco-tales)<br>
Без внешнего баффа Силы атаки это оружие лучше, чем Солнечная жемчужина или Песнь странника, но с баффом это оружие проигрывает остальным.

**О комплектах артефактов**

[Встречная комета (4)](#artifact:retracing-bolide)<br>
Комета дает дополнительно 40% урона обычной и Заряженной атакам, когда персонаж в щите. Кристаллизация не такой надежный источник щитов, как хотелось бы, поэтому лучше использовать героя-щитовика, например, Чжуна Ли.


# === characters.#ningguang.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#ningguang.roles.#burst_support.tips ===

Пройдите через Нефритовую ширму, созданную Элементальным навыком Нин Гуан перед активацией Взрыва стихии для получения бонуса Гео урона, предоставленного её пассивным талантом 60+ уровня.


# === characters.#ningguang.roles.#burst_support.notes ===

Ульт-саппорт Нин Гуан почти не пользуется обычными и Заряженными атаками, и наносит свой основной урон Элементальным навыком и Взрывом стихии.

**О выборе оружия**

Рейтинг сформирован с учетом наличия внешнего баффа Силы атаки Нин Гуан.

[Молитва святым ветрам](#weapon:lost-prayer-to-the-sacred-winds)<br>
Чуть хуже Небесного атласа из-за невозможности нормально собрать стаки бонуса, но благодаря тому, что это оружие на Криты, становится проще билдить персонажа.

[Память о пыли](#weapon:memory-of-dust)<br>
Лучшее оружие, когда у Нин Гуан нет никакого внешнего баффа Силы атаки.

[Солнечная жемчужина](#weapon:solar-pearl)<br>
Так как бонус Жемчужины значительно увеличивает урон Взрыва стихии, с внешними баффами Силы атаки это оружие становится равным Памяти о пыли.


# === characters.#noelle.roles.#dps.name ===

ДД


# === characters.#noelle.roles.#dps.weapons.notes ===

предполагается наличие \[С6\]


# === characters.#noelle.roles.#dps.mainStats.notes ===

Часы на процент атаки в приоритете до \[С6\] и 9 уровня Взрыва стихии


# === characters.#noelle.roles.#dps.subStats.advices.1.notes ===

\~20-30%


# === characters.#noelle.roles.#dps.tips ===

Оптимальная последовательность атак: три-четыре обычных атаки, затем прыжок или сприт, чтобы начать серию атак сначала
<br>


# === characters.#noelle.roles.#dps.notes ===

До \[С6\] не рекомендуется собирать ДД Ноэлль. На \[С6\] половина ее Защиты конвертируется в Силу атки. С её щитами, возможностью лечения, уроном по площади, она может быть хорошим ДД при построении правильного отряда вокруг неё.

**О выборе оружия**

[Меч драконьей кости](#weapon:serpent-spine)<br>
С комплектом Кокона сладких грёз, стаки бонуса от Меча драконьей кости важнее стаков Кокона.

[Небесное величие](#weapon:skyward-pride)<br>
Полезный меч, когда в допстатах недостаточно Восстановления энергии. Наносит хороший урон, если активен бонус оружия.

[Двуручный меч Фавония](#weapon:favonius-greatsword)<br>
Полезен только на больших рангах пробуждения, потому что с более высоким рангом генерирует больше частиц. Р5 позволяет играть без батарейки, но второй Гео персонаж всегда полезен для резонанса.

**О комплектах артефактов**

[Кокон сладких грёз (4)](#artifact:husk-of-opulent-dreams)<br>
Взрыв стихии Ноэль снапшотает Защиту. Чтобы получить все 4 стака бонуса Защиты с этого комплекта, нужно подержать Ноэль в кармане 12 секунд. В Бездне время до начала испытания тоже считается.

[Встречная комета (4)](#artifact:retracing-bolide)<br>
Даёт чуть меньше урона, чем Конец гладиатора, но Комету проще собрать, и Ноэль всегда легко поддерживает активным бонус комплекта используя щиты от реакции Кристалл. Конец гладиатора можно рассматривать, если не хочется добывать [Архаичный камень](#artifact:archaic-petra). По расходу смолы выгоднее добывать Конец гладиатора не с боссов, а через Мистическое подношение алхимического стола.


# === characters.#noelle.roles.#support.name ===

Саппорт


# === characters.#noelle.roles.#support.artifacts.notes ===

Главное собрать максимальное количество защиты, бонус сета не так важен.


# === characters.#noelle.roles.#support.tips ===

Для лечения следует использовать её Заряженные атаки, так как они быстрее.


# === characters.#noelle.roles.#support.notes ===

На данный момент Ноэль не рекомендуется собирать в саппорта из-за плохого аптайма щита и слабого лечения.

**О выборе оружия**

[Церемониальный двуручный меч](#weapon:sacrificial-greatsword)<br>
Даже на \[Р5\], время отката 16 секунд, когда как длительность щита Ноэлль 12, поэтому щит будет не всегда активен.

[Двуручный меч Фавония](#weapon:favonius-greatsword)<br>
Требует Критов для генерации энергии. Откат в 12 секунд \[Р1\] совпадает со временем отката её щита. В целом не рекомендуется, можно использовать только если есть избыток Критов в допстатах.


# === characters.#qiqi.roles.#support.name ===

Саппорт


# === characters.#qiqi.roles.#support.notes ===

Взрыв стихии Ци Ци стоит 80 энергии, а её Элементальный навык не восстанавливает энергию до \[С1\]. Восстановление энергии хорошо помогает Ци Ци, а её Взрыв стихии должен использоваться в крайнем случае.

Ци Ци хорошо лечит будучи неактивной, благодаря своему Взрыву стихии и пассивному навыку 60 уровня.

**О выборе оружия и артефактов**

[Церемониальный меч](#weapon:sacrificial-sword) и [ Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)<br>
Стойкость Миллелита на Ци Ци дает хороший бафф Силы атаки отряду. Церемониальный меч увеличивает время активности её Элементального навыка, что в свою очередь увеличивает время действия баффа Силы атаки от артефактов. Если Ци Ци лечит слишком сильно, можно переодеть её в этот комплект. Но стоит учитывать, что Церемониальный меч на некоторых других персонажах будет полезнее.

[Моллюск морских красок (4)](#artifact:ocean-hued-clam)<br>
Позиция в рейтинге основывается на том, что Ци Ци не часто использует Взрыв стихии и лечит преимущественно своим Элементальным навыком. В отряде, где она часто использует Взрыв стихии, этот комплект превзойдет Стойкость Миллелита (4).


# === characters.#raiden-shogun.credits ===

[Подробный разбор](https://keqingmains.com/raiden/) от Райдэн мейнеров и ККМ (англ.)


# === characters.#raiden-shogun.roles.#dps.name ===

ДД


# === characters.#raiden-shogun.roles.#dps.tips ===

Следует начинать бой с Элементального навыка Райден для усиления Взрывов стихии отряда.

Последовательность атак после Взрыва стихии Райден:

Рекомендуется:<br>
Элементальный навык > Взрыв стихии > два раза по 4ОА + ЗА > 2ОА + ЗА<br>
Оптимально: Элементальный навык > Взрыв стихии > три раза по 3ОА + ЗА > 1ОА + ЗА

\*ОА — обычная атака, ЗА — Заряженная атака

Рекомендуемая последовательность оставляет больше поля для ошибок и даёт чуть больше урона по площади. Оптимальная последовательность дает немного больший урон.


# === characters.#raiden-shogun.roles.#dps.notes ===

Урон Взрыва стихии Райдэн зависит от стоимости Взрыва стихий членов отряда. Её взрыв Стихии восстанавливает большое количество энергии отряду. Это особенно полезно в командах, требующих много энергии и не конфликтующих с Электро, например с Эолой, в Электро-националке и так далее. Райдэн ультует по откату с 200% Восстановления энергии, но можно поднять процент выше, чтобы лучше восстанавливать энергию отряду.

**О выборе оружия**

Список основывается на том, что Райден играется без баффера.

**[Режущий волны плавник](#weapon:wavebreakers-fin)**<br>
\[Р5\] Режущий волны плавник примерно равен Посоху Хомы, зависит от общей стоимости Взрыва стихий отряда.

**О комплектах артефактов**

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)**<br>
Этот комплект для билда саппорта с маленькими инвестициями. Райдэн хорошо подходит на роль носителя этого комплекта, потому что её Элементальный навык имеет 100% аптейм. Обратите внимание, что если Райдэн носит этот комплект, то она теряет в уроне.

**Об основных характеристиках**

Главстаты кубка и часов зависят от допстатов, наличия внешнего баффера, используемого оружия. [Больше рекомендаций (англ.)](https://docs.google.com/spreadsheets/d/1VdRUAKFUoKkceIghV5tCfr3kfoiH59mcFOMorI9jYrw/edit#gid=1372936118)


# === characters.#razor.credits ===

[Подробный разбор](https://keqingmains.com/razor/) от Рэйзор мейнеров и ККМ (англ.)


# === characters.#razor.roles.#dps.name ===

ДД


# === characters.#razor.roles.#dps.tips ===

Взрыв стихии Рэйзора накладывает на него Электро статус во время обычных атак. Это можно использовать для очистки других статусов с себя, но со слабой эффективностью. Так можно очистить статус только с Рэйзора.


# === characters.#razor.roles.#dps.notes ===

**О выборе оружия**

**[Волчья погибель](#weapon:wolfs-gravestone)**<br>
Волчья погибель превосходит Небесное величие во всех случаях, кроме битв с большим количеством врагов.

**[Краснорогий камнеруб](#weapon:redhorn-stonethresher)**<br>
В зависимости от того, сколько баффов на Силу атаки получает Рэйзор, это оружие может обогнать Волчью погибель по урону.

**[Некованый](#weapon:the-unforged)**<br>
Игра с этим оружием более комфортна, потому что не нужно следить за баффами Волчьей погибели или Песни разбитых сосен для доп. ДД, при этом дает бонус на время действия щита. Чжун Ли лучший вариант щитовика в команду.

**[Меч драконьей кости](#weapon:serpent-spine)**<br>
На \[С6\] высоком ранге пробуждения Меча драконьей кости, лучше взять кубок на процент Силы атаки, чем на процент Физического урона. Из-за того, что большая часть урона Рэйзора наносится Электро. [Подробнее о причинах (англ.)](https://tinyurl.com/razoratk) Стоит так же отметить, что **\[Р5\] Меч драконьей кости максимальными стаками превосходит всё \[Р1\] пятизвездочное оружие.**

**[Небесное величие](#weapon:skyward-pride)**<br>
Небесное величие хорошо сочетается с кубком на Физический урон, так как урон от бонуса оружия зависит от физ. урона.

**О комплектах артефактов**

**[Бледный огонь (2) ](#artifact:pale-flame)+ [Рыцарь крови (2)](#artifact:bloodstained-chivalry)**<br>
Этот набор рекомендуется только тем, у кого еще нет возможности экипировать Бледный огонь (4). Так же этот комплект лучше Конца гладиатора (4), но  только если есть хороший процент Силы атаки в допстатах.


# === characters.#rosaria.roles.#cryo_dps.name ===

Крио ДД


# === characters.#rosaria.roles.#cryo_dps.notes ===

**О допстатах**

Розария не нуждается в большом количестве Восстановления энергии, если находится в паре с Кэйей. В паре с другим Крио, например Чун Юнем или Аякой, ей нужно 130-150% Восстановления энергии.

По сравнению с Кэйей, Розарии еще менее важен Шанс крит. попадания, так как её первый пассивный талант даёт 12% Шанса крит. попадания.

**О выборе оружия**

[Режущий волны плавник](#weapon:wavebreakers-fin)<br>
На \[Р5\], это оружие идентично Посоху Хомы.

[Смертельный бой](#weapon:deathmatch)<br>
Розария получает много Шанса крит. попадания от комплекта Заблудшего в метели (4) и Крио резонанса. Поэтому это оружие не так ценно, ведь с ним Розария получает более 100% Шанса крит. попадания. Если Розария не использует комплект Заблудшего в метели (4), то это оружие может быть лучше других.

**О приоритетах талантов**

**Обычная атака**<br>
Если Розария играется через накладывание Крио инфузии на обычные атаки с помощью Чун Юня, то прокачка её обычной атаки важнее, чем улучшение Элементального навыка.


# === characters.#rosaria.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#rosaria.roles.#burst_support.tips ===

На \[С1+\] следует использовать Элементальный навык перед Взрывом стихии, чтобы увеличить бонусный урон.


# === characters.#rosaria.roles.#burst_support.notes ===

**О выборе оружия**

Рейтинг посчитан исходя из того, что Розарии нужно 160% Восстановления энергии, поэтому оружие с бонусом Восстановления энергии оценены выше. На \[С4\] требуемое Восстановление энергии Розарии падает до 140% или ещё ниже.

**[Покоритель вихря](#weapon:vortex-vanquisher)**<br>
Позиция в рейтинге с учетом отсутствия щита. С большим количество баффов на атаку, например, с Беннетом, это оружие станет хуже других из-за слишком больших процентов Силы атаки.

**[Гроза драконов](#weapon:dragons-bane) / [Крест-копьё Китаин](#weapon:kitain-cross-spear)**<br>
Это оружие подходит только для отрядов, в которых Розария часто вызывает Реверсивное Таяние. \[Р5\] Гроза драконов может быть лучше, чем [«Улов»](#weapon:the-catch), и \[Р5\] Крест-копьё Китаин находится наравне со Смертельным боем.

**[Режущий волны плавник](#weapon:wavebreakers-fin)**<br>
На \[Р5\] это оружие такое же, как Посох Хомы.

**[Каменное копьё](#weapon:lithic-spear)**<br>
На \[Р5\] это оружие такое же, как [Покоритель вихря](#weapon:vortex-vanquisher).

**[Копьё Фавония](#weapon:favonius-lance)**<br>
Хоть его урон для Розарии и ниже, дополнительная энергия от пассивки может оказаться очень полезной для подзарядки энергии Крио (Эоле, Аяке и др.).

**О комплектах артефактов**

[Ступающий по лаве (4)](#artifact:lavawalker)<br>
Подходит только для отрядов, в которых Розария часто вызывает Реверсивное Таяние, тогда этот комплект лучше, чем набор Церемония древней знати (2) и Заблудший в метели (2).

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Даёт бонус урона всему отряду, можно использовать на Розарии, если больше никто в отряде не носит этот комплект.

**О допстатах**<br>
Мастерство стихий рекомендуется только если Розария вызывает Реверсивное Таяние.

Те, кто не собрал хороший комплект и хочет использовать Розарию для баффа на Шанс крит. попадания, или не хотят инвестировать в неё, могут взять любое копье и артефакты с Шансом Крит. попадания.


# === characters.#sayu.roles.#support.name ===

Саппорт


# === characters.#sayu.roles.#support.tips ===

Чтобы сгенерировать больше частиц, удерживайте кнопку Элементального навыка, а затем сразу же нажмите её ещё раз.


# === characters.#sayu.roles.#support.notes ===

Саю получает бонус урона от своего Мастерства стихий и бонус лечения от своей Силы атаки. Можно повысить лечение с помощью короны с Бонусом лечения, сохранив при этом приемлимый урон.Также Саю требуется большое количество Восстановления энергии (около 200%), если она — единственный Анемо персонаж в отряде.

**О выборе оружия**

[Волчья погибель](#weapon:wolfs-gravestone)<br>
Универсальный двуручник, который и баффает всю команду, и, в случае с Саю, повышает лечение. Его лучше всего использовать, когда уже достигнут максимум Восстановления энергии.

**О комплектах артефактов**

[Изумрудная тень](#artifact:viridescent-venerer)<br>
Топовый комплект артефактов для Анемо персонажей, он может понижать вражеское сопротивление к элементам, повышая в то же время урон Саю от Рассеивания.


# === characters.#shenhe.roles.#support.name ===

Саппорт


# === characters.#shenhe.roles.#support.notes ===

Шэнь Хэ  — героиня поддержки для Крио персонажей. Она баффает Крио урон при использовании своего Элементального навыка, наносит урон и снижает Крио и физ. сопротивление при использовании Взрыва стихии. Её **Элементальный навык даёт несколько зарядов бонусного урона для всех окружающих членов отряда (5 зарядов при быстром нажатии и 7 зарядов при долгом).** Этот бонус урона **работатет только для Крио атак,** так что персонажи, которые не наносят Крио урон, не получат бонус от Элементального навыка Шэнь Хэ. Обратите внимание, что **Крио инфузия от Анемо персонажей считается Крио атакой, следовательно получает бонус от её Элементального навыка.** Другой факт, на который стоит обратить внимание, это то, что бафф урона Силы атаки, который Шэнь Хэ даёт отряду, не снапшотает.

**О выборе оружия**

[Усмиритель бед](#weapon:calamity-queller)<br>
Пассивная способность этого оружия идеально работает с Элементальным навыком Шэнь Хэ. Это оружие также имеет высокую Базовую атаку.

[Сияющая жатва](#weapon:engulfing-lightning)<br>
Это оружие обеспечивает Восстановление энергии и конвертирует Восстановление энергии в Силу атаки, что полезно для Шэнь Хэ.

[Покоритель вихря](#weapon:vortex-vanquisher)<br>
Пассивка этого оружия не может быть заряжена из кармана, но оно все еще имеет допстат, который обеспечивает Шэнь Хэ большим количеством Силы атаки.

[Копьё Фавония](#weapon:favonius-lance)<br>
С этим оружием лучше иметь некоторое количество Шанса крит. попадания для активации пассивки этого оружия. Обратите внимание, что Крио резонанс увеличивает на 15% Шанс крит. попадания по замороженным врагам или врагам с Крио эффектом, так что потребность Шанса крит. попадания у Шэнь Хэ не особенно высокая.

**О приоритете основных характеристик**

При экипировании оружия с Силой атаки, лучшим выбором станут часы на Восстановление энергии и наоборот.


# === characters.#sucrose.roles.#em_build.name ===

Сборка через МС


# === characters.#sucrose.roles.#em_build.talents.notes ===

Прокачивать таланты необязательно


# === characters.#sucrose.roles.#em_build.tips ===

При использовании полного комплекта [Изумрудной тени (4)](#artifact:viridescent-venerer) важно **не переключаться на другого персонажа сразу** после активации Элементального навыка или Взрыва стихии: Сахароза должна находиться на поле боя в момент поглощения, чтобы сработал эффект комплекта.


# === characters.#sucrose.roles.#em_build.notes ===

Талант 60 уровня Сахарозы повышает Мастерство стихий отряду на 20% от суммарного Мастерства стихий самой Сахарозы, поэтому стоит набрать артефактами как можно больше Мастерства стихий. В роли саппорта, главные таланты Сахарозы это пассивные таланты 20 и 60 уровней, поэтому прокачка умений — обычной атаки, Элементального навыка и Взрыва стихии — не обязательна.

**О выборе оружия**

[Эпос о драконоборцах](#weapon:thrilling-tales-of-dragon-slayers)<br>
Не даёт бонуса к Мастерству стихий, но услиливает отряд лучше остальных. При переключении персонажа оно даёт бонус 48% Силы атаки, что эквивалентно легендарным часам на Силу атаки 20 уровня. То есть Эпос позволяет наносить больше урона по сравнению [Церемониальными мемуарами](#weapon:sacrificial-fragments), которые дают всего \~44 EM (эквивалентно 2 допстатам куска артефакта).

[Кольцо Хакусин](#weapon:hakushin-ring)<br>
Хороший выбор для команд, в которых Сахароза может вихрем поглотить Электро и вызвать у оружия пассивку с бонусом урона. В отрядах, играющих через реакцию Заряжен, Сахароза может баффнуть и процент Гидро урона.

[Кодекс Фавония](#weapon:favonius-codex)<br>
Это оружие будет полезно, если от Сахарозы нужно больше частичек. Чтобы чаще актривировать бонус оружия, Сахарозе необходимо собрать побольше Шанса крит. попадания.

В некоторых отрядах от Сахарозы не важен бонус Мастерства стихий, например, в командах Заморозки или где она используется как Анемо-батарейка. В таких отрядах лучше использовать [Эпос о драконоборцах](#weapon:thrilling-tales-of-dragon-slayers)


# === characters.#tartaglia.credits ===

[Подробный разбор](https://tinyurl.com/4nnv5euy)  от Тарталья мейнеров


# === characters.#tartaglia.roles.#dps.name ===

ДД


# === characters.#tartaglia.roles.#dps.tips ===

Оптимальная продолжительность умения на  \[C0\]: 7 с, 9 с или 12 с.<br>
Оптимальная продолжительность умения на \[С1\]: 9 с, 11 с или 14 с.

**Взрыв стихии в Ближнем бою** <br>
Используется только тогда, когда нужна одна ротация до Взрыва стихии, потому что взрыв стихии в дальнем бою лучше восстанавливает энергию.

**Последовательность атак в ближнем бою**<br>
Оптимально:2 ОА + 1 ЗА (Оптимально)<br>
Рекомендуется: 3 ОА + 1 ЗА (Рекомендуется)<br>
Рекомендуется с Ржавым луком или Громовым пульсом: 5 ОА + 1 ЗА

\*ОА — обычная атака, ЗА — Заряженная атака

Оптимальная последовательность дает чуть больше урона, но тратит больше выносливости. Рекомендуемая последовательность наоборот.


# === characters.#tartaglia.roles.#dps.notes ===

Рейтинг оружия основывается на том, что Тарталья используется с Беннетом, вызывает Пар на своем Взрыве стихий и бьет оптимальной последовательностью атак.

**О выборе оружия**

**[Полярная звезда](#weapon:polar-star)**<br>
Полезность зависит от того, сколько удастся собрать стаков. В случае игры в комманде с Беннетом, то, если начать бой с 2-3 стаками оружие будет полезнее Громолвого пульса, если начать без стаков — оружие будет хуже, чем Небесное крыло. В отряде без Беннета, Полярная звезда всегда лучше другого оружия. Три стака можно собрать, если выполнить Элементальный навык, затем одну обычную и одну заряженную атаку. Если Элементальный навык в откате, то полностью заряженный прицельный выстрел против цели под воздействием Отлива, даст два стака. Также по одному стаку дадут обычный и прицельный выстрел из лука.

**[Громовой пульс](#weapon:thundering-pulse)**<br>
Слегка лучше, чем Небесное крыло в зависимости от стиля боя — чем больше времени проводятся обычные атаки, тем лучше этот лук. Но не рекомендуется увеличивать время, проводимое в ближнем бою ради этого оружия.

**[Зелёный лук](#weapon:the-viridescent-hunt)**<br>
Имеет полезную пассивную способность, помогающую собирать врагов в кучки, что позволяет наносить тонны урона Отливом.

**[Черногорский боевой лук](#weapon:blackcliff-warbow)**<br>
Из-за сложности сбора стаков, хуже, чем остальное оружия с большим рангом пробуждения.

**[Прототип: Полумесяц](#weapon:prototype-crescent)**<br>
Заставляет активировать пассивную способность атаками по слабому месту врага. Когда пассивная способность не активирована, это оружие хуже других.

**[Бесструнный](#weapon:the-stringless)**<br>
Тарталье следует использовать это оружие только в отрядах, где он вызывает Испарение на свой Взрыв стихии, потому, что в этих отрядах полезен допстат на Мастерство стихий. Если Тарталья используется только как Взрыв саппорт, Бесструнный \[Р5\] хуже, чем Громовой пульс. Также следует обратить внимание, что бонус Бесструнного не активируется Тартальей в стойке ближнего боя.

**О комплектах артефактов**

**[Сердце глубин (2) ](#artifact:heart-of-depth)и [18% Силы атаки (2)](#artifact:18%-atk)**<br>
Этот комплект не отстает от набора Сердце глубин (4), можно остановиться на нем, если удалось собрать достойные куски.

**[Воспоминания Симэнавы (4)](#artifact:shimenawas-reminiscence)**<br>
Из-за того, что этот комплект требует иметь около 140% Восстановления энергии, чтобы все время активировать Взрыв стихии, он хуже других комплектов. Но он выше по урону, чем Сердце глубин (4), если сравнивать ротацию с одной ультой, и не собирать Восстановление энергии.


# === characters.#thoma.roles.#support.name ===

Саппорт


# === characters.#thoma.roles.#support.subStats.notes ===

Шанс крит. попадания важнее для [Копья Фавония](#weapon:favonius-lance).


# === characters.#thoma.roles.#support.notes ===

В идеале, щит Томы приоритетнее урона, и важно чаще использовать Взрыв стихии для обновления щита, поэтому для Томы важнее Восстановление энергии, чем процент ХП.

**О выборе оружия**

[Чёрная кисть](#weapon:black-tassel)<br>
На \[С4\], Тома получает 15 энергии, когда активирует свой Взрыв стихии. Этот позволяет ему использовать Чёрную кисть с часами на Восстановление энергии, что еще больше усиляет его щит, вместо экипирования и оружия, и часов на Восстановление энергии.


# === characters.#traveler-anemo.roles.#anemo_dps.name ===

Анемо ДД


# === characters.#traveler-anemo.roles.#anemo_dps.subStats.notes ===

Бонусы Шанса крит. попадания и Крит. урона приоритетнее для сборки через криты


# === characters.#traveler-anemo.roles.#anemo_dps.notes ===

В этой сборке Анемо Путешественница является саппортом, и использует только свой Элементальный навык и Взрыв стихии.

Анемо Путешественнице нужно до 160% Восстановления энергии.

**О выборе оружия**

Анемо Путешественница может выбрать либо билд на Мастерство стихий, либо на Силу атаки или Криты без особой разницы в наносимом уроне. Выбранный билд не влияет на рейтинг оружия.

[Церемониальный меч](#weapon:sacrificial-sword)<br>
Позиция в рейтинге основана на предположении о том, что Путешественница все время использует Элементельный навык после бонуса меча. Но для этого ей придётся оставаться на поле боя гораздо дольше, что иногда может быть нежелательно.

[Амэнома Кагэути](#weapon:amenoma-kageuchi)<br>
Понижает потребность Восстановления энергии до примерно 135% на \[Р1\] и 110% на \[Р5\]. На \[Р5\] это оружие будет лучше, чем [Церемониальный меч](#weapon:sacrificial-sword).

[Меч Фавония](#weapon:favonius-sword)<br>
Хотя этот меч и не даёт много урона, его пассивная генерация частиц энергии иногда может быть полезнее для отряда в целом. Нужно только убедиться, что шанса Крита в билде хватит для надёжного вызова пассивки.


# === characters.#traveler-anemo.roles.#physical_dps.name ===

Физ. ДД


# === characters.#traveler-anemo.roles.#physical_dps.notes ===

Подходит только для новичков.

**О комплектах артефактов**

[Бледный огонь (4)](#artifact:pale-flame)<br>
С этим комплектом следует использовать быстрый Элементальный навык, не удерживая кнопку. Иначе из-за долгого отката бонус комплекта не сможет накапливаться.


# === characters.#traveler-electro.roles.#support.name ===

Саппорт


# === characters.#traveler-electro.roles.#support.mainStats.notes ===

Главстаты не очень важны. Лучше собрать достаточно Восстановления энергии для частого Взрыва стихии.


# === characters.#traveler-electro.roles.#support.tips ===

Не рекомендуется повышать таланты Элементального навыка и Взрыва стихии выше 7 уровня: количество генерируемой энергии увеличиваться уже не будет.


# === characters.#traveler-electro.roles.#support.notes ===

Путешественница может дать много энергии и Восстановления энергии активным персонажам, если подбирать Амулеты изобилия, созданные её Элементальным навыком и использовать Взрыв стихии. Особенно хорошо это подходит для ДД которые не конфликтуют с Электро и нуждаются в заливе энергии. Из-за плохих скейлов Электро Путешественницы, не рекомендуется повышать уровень её талантов, стоит только собирать Восстановление энергии для усиления способностей батарейки.


# === characters.#traveler-geo.roles.#geo_dps.name ===

Гео ДД


# === characters.#traveler-geo.roles.#geo_dps.tips ===

Элементальный навык и Взрыв стихии имеют одинаковый приоритет.<br>
Лучше использовать Элементальный навык через долгое нажатие, потому что у быстрого нажатия есть небольшая задержка активации.


# === characters.#traveler-geo.roles.#geo_dps.notes ===

В этом билде Путешественница наносит урон Взрывом стихии, остальное время будучи неактивной.

**О выборе оружия**

**[Осквернённое желание](#weapon:festering-desire), [Небесный меч](#weapon:skyward-blade), [Амэнома Кагэути](#weapon:amenoma-kageuchi)**<br>
Эти мечи уменьшают время отката Взрыва стихии. Если Восстановления энергии достаточно и без них, то они проигрывают по урону другим вариантам.


# === characters.#traveler-geo.roles.#physical_dps.name ===

Физ. ДД


# === characters.#traveler-geo.roles.#physical_dps.artifacts.notes ===

Cм. заметки к [Бледному огню (4)](#artifact:pale-flame)


# === characters.#traveler-geo.roles.#physical_dps.notes ===

Подходит только для новичков.

**О комплектах артефактов**

[Конец гладиатора (2) Бледный огонь (2)](#artifact:gladiators-finale)<br>
Этот набор лучше, чем Бледный огонь (2) и Рыцарь крови (2) из-за количества Гео урона который может нанести Путешественница.

[Бледный огонь (4)](#artifact:pale-flame)<br>
Этот комплект не рекомендован по причине того, что время отката Элементального навыка Путешественницы 6 секунд, из-за чего сложно поддерживать активным полный бонус комплекта.


# === characters.#venti.credits ===

[Подробный разбор](https://keqingmains.com/venti/) от ККМ (англ.)


# === characters.#venti.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#venti.roles.#off-field_dps.subStats.notes ===

Предполагается, что на персонаже экипированы и часы, и кубок, и корона на Мастерство стихий, при этом Шанс крит. попадания и Крит. урон не очень важны.


# === characters.#venti.roles.#off-field_dps.talents.notes ===

Для сборки на Мастерство стихий прокачивать таланты необязательно.


# === characters.#venti.roles.#off-field_dps.tips ===

Чтобы сработал бонус комплекта [Изумрудная тень](#artifact:viridescent-venerer), владелец должен быть на поле боя когда будет вызываться реакция Рассеивания. Поэтому, после использования Взрыва стихии, нужно подождеть первого рассеивания, а не сразу же переключаться на другого персонажа. Или же можно использовать Элементальный навык перед Взрывом стихий.

Можно наложить элементальную инфузию на Взрыв стихии Венти, если зайти в нее персонажем с наложенным элементальным статусом. Например, использовать Взрыв стихии Беннета, затем Взрыв Венти, затем зайти внутрь чтобы наложить на ульту Венти Пиро инфузию.


# === characters.#venti.roles.#off-field_dps.notes ===

В этом билде урон предпочитается Восстановлению энергии, из-за пассивного таланта Венти на Восстановление энергии.

Рейтинг оружия основан на том, что Венти собирает Мастерство стихий, так как собирать Криты слишком дорого и не нужно для битв с группами врагов.

**О выборе оружия**

[Боевой лук Фавония](#weapon:favonius-warbow)
Несмотря на ужасный собственный урон, с этим луком Венти даёт больше энергии себе и команде. С этим оружием рекомендуется собрать побольше Шанса крит. попадания, чтобы чаще активировать бонус лука.

[Небесное крыло](#weapon:skyward-harp)
В билде на Мастерство стихий статы этого оружия не ценны. Если хочется играть через криты, то рекомендуется углубленное изучение [гайда (англ.)](https://keqingmains.com/venti/)

[Элегия погибели](#weapon:elegy-for-the-end)
Бонус Элегии погибели баффает Взрыв стихии Венти на силу атаки, но баффает Мастерство стихий - ?. Это происходит потому, что Взрыв стихии снапшотает урон. Хотя с \[Р1\] Элегией погибели у Венти чуть ниже собственный урон по сравнению с \[Р5\] Бесструнным, примерно на 7% если бафф Элегии активен только половину времени, бафф Элегии обеспечивает бонус Силы атаки и Мастерства стихий всему отряду, что повышает общий урон отряда.


# === characters.#xiangling.credits ===

[Подробный разбор](https://keqingmains.com/xiangling/) от ККМ (англ.)


# === characters.#xiangling.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#xiangling.roles.#off-field_dps.tips ===

Частоиспользуемые отряды для Пиро Сян Лин:

**Националка:** Сян Лин, Син Цю, Беннет и любой персонаж (Сахароза/Чжун Ли/Кадзуха/Райдэн/Чун Юнь/и т.д)

**Отряд Тартальи:** Тарталья, Сян Лин, Беннет и любой персонаж (Сахароза/Чжун Ли/Венти/Кадзуха/и т.д)

_**Рекомендуемый порог Восстановления энергии**_

**Националка/Отряд Тартальи:** 160-200% Восстановления энергии<br>
**Активатор** для отрядов **Реверсивного Таяния:** 200-250% Восстановления энергии


# === characters.#xiangling.roles.#off-field_dps.notes ===

**О выборе оружия**

Этот список основывается на том, что **Сян Лин** используется **и с Беннетом, и со вторым баффером** (то есть Кадзухой/Сахарозой)

[Режущий волны плавник](#weapon:wavebreakers-fin)<br>
\[Р5\] Режущий волны плавник равен или превосходит Посох Хомы, в зависимости от общей стоимости Взрыва стихии вашей команды.

[Гроза драконов](#weapon:dragons-bane)<br>
\[Р5\] Гроза драконов превосходит Нефритового коршуна.

[Крест-копьё Китаин](#weapon:kitain-cross-spear)<br>
Пассивное умение Крест-копья Китаина значительно снижает порог Восстановления энергии Сян Лин, до \~140% Восстановления энергии на \[Р1\] и \~110% Восстановления энергии на \[Р5\]. Это позволяет выбрать артефакты с атакующими допстатами, вместо допстатов на Восстановление энергии, и благодаря этому \[Р5\] Крест-копьё Китаин может превзойти Нефритового коршуна. Из-за значительного снижения порога Восстановления энергии комплект артефактов Горящая алая ведьма (4)  превзойдет Эмблему рассечённой судьбы (4).

[Каменное копьё](#weapon:lithic-spear)<br>
\[Р5\] Каменное копьё со 2 бонусами работает так же, как Нефритовый коршун.

[Копьё Фавония](#weapon:favonius-lance)<br>
Полезность, обеспечиваемая **копьем Фавония с высоким рангом пробуждения** может сделать его предпочтительным оружием в отрядах где Сян Лин не вызывает реакции, а только накладывает Пиро, например **в отрядах Реверсивного Таяния.**

**Об оружии и Артефактах со статом Мастерства стихий**

[Гроза драконов](#weapon:dragons-bane)/[Крест-копьё Китаин](#weapon:kitain-cross-spear)**/Мастерство стихий в артефактах**<br>
В отрядах, где можно вызывать и Пар и Перегрузку одновременно (то есть Националка Райдэн), Мастерство стихий ценится больше, выводя это оружие и артефакты вперед по наносимому урону.

**О комплектах артефактов**

**[Горящая алая ведьма (4)](#artifact:crimson-witch-of-flames)**<br>
Этот комплект предпочтительнее [Эмблемы рассечённой судьбы (4)](#artifact:emblem-of-severed-fate) в отрядах, где Сян Лин вызывает Перегрузку и Испарение (то есть Националка Райдэн).

**[Изгнанник (2) Учёный (2)](#artifact:the-exile)**<br>
Этот комплект может быть использован как **очень бюджетная саппорт сборка для построения отрядов Реверсивного Таяния,** где Сян Лин не вызывает реакции и вместо этого используется для наложения Пиро и требует много Восстановления энергии. Обратите внимание на то, что **не рекомендуется останавливаться на этой сборке, так как Сян Лин будет наносить больше урона с более качественными артефактами.**


# === characters.#xiangling.roles.#physical_dps.name ===

Физ. ДД


# === characters.#xiangling.roles.#physical_dps.notes ===

**Не рекомендовано использовать Сян Лин как физ. ДД, поскольку Сян Лин восхитительна в роли карманного Пиро ДД.** Физ Сян Лин хороша только для ранней игры.

**О выборе оружия**

[Пика полумесяца](#weapon:crescent-pike)<br>
Пика полумесяца очень сильна на Сян Лин благодаря особенностям ее комбо-атак, Пика полумесяца превосходит Нефритового коршуна в отношении урона с обычных/Заряженных атак.

**О комплектах артефактов**

[Бледный огонь (4)](#artifact:pale-flame)<br>
Пассивка этого комплекта может быть всегда активна при использовании Гобы по откату.


# === characters.#xiao.credits ===

[Подробный разбор](https://keqingmains.com/xiao/) от Сяо мейнеров и ККМ (англ.)


# === characters.#xiao.roles.#dps.name ===

ДД


# === characters.#xiao.roles.#dps.tips ===

**Оптимальная последовательность атак Сяо**<br>
Элементальный навык > Элементальный навык > Взрыв стихии > Атаки в падении

**Во время взрыва стихий**

**Лучшая последовательность против одного противника**<br>
Атака в падении > обычная атака > Заряженная атака > Атака в падении

**Лучшая последовательность против нескольких противников**<br>
Атака в падении > обычная атака > атака в падении


# === characters.#xiao.roles.#dps.notes ===

У Сяо есть проблемы с восстановлением энергии из-за того, что он не может генерировать частицы с помощью Элементального навыка, пока активен его Взрыв стихии. Поэтому не рекомендовано использовать Элементальный навык пока длится Взрыв стихии.

Следует начинать бой с использования навыка Анемо батарейки, после чего использовать Элементальный навык Сяо дважды, затем сразу же активировать Взрыв стихии. Тогда прилетевшие частицы зарядят будущий Взрыв.

Следует использовать Сяо вместе с другим Анемо персонажем, например Венти, Сахарозой, Джинн или Анемо Путешественницей, чтобы быстрее накапливать ему Взрыв стихий. Его Взрыв стихии заканчивается при переключении на другого персонажа. У Элементального навыка Сяо есть два заряда, они откатываются по одному.

**О выборе оружия**

С хорошей Анемо батарейкой, например Сахарозой, Сяо нужно 120-130% Восстановления энергии чтобы поддерживать Взрыв стихии всё время активным. Если Восстановления энергии достаточно, то не стоит брать оружие с Восстановлением энергии, чтобы не потерять в уроне.

**[Усмиритель бед](#weapon:calamity-queller)**<br>
У этого оружия высокая Базовая атака, что подразумевает получение большего преимущества от баффов Силы атаки, например, от Эпоса о драконоборцах.

**О комплектах артефактов**

Изумрудная тень лучше второго комплекта на 18% Силы атаки, если у Сяо слишком много баффов на Силу атаки.

Следует обратить внимание, что у Сяо много собственных баффов на Силу атаки, и комплект артефактов на +18 Силы атаки дает ему не так много. Поэтому можно пренебречь бонусом комплекта и собрать любые артефакты с хорошими допстатами.


**О главстате артефактов**

Для Сяо рекомендуются кубки с бонусом процента Анемо урона. Кубки с процентом Атаки будут неплохой альтернативой.


# === characters.#xingqiu.credits ===

[Подробный разбор](https://docs.google.com/document/d/1RQI5IjLZKqsVhQzRDr7FTYgQX66jKB28PnwoZVlQ4_k/edit) от Quincy#6965 (англ.)


# === characters.#xingqiu.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#xingqiu.roles.#off-field_dps.subStats.notes ===

В первую очередь, стоит прокачивать Восстановление энергии до тех пор, пока не получится активировать Взрыв стихии по откату.


# === characters.#xingqiu.roles.#off-field_dps.tips ===

Активация Элементального навыка или Взрыва стихии Син Цю накладывает Гидро на активного персонажа после переключения. Таким образом, можно снимать статусы с персонажей.

Снятие статуса Электро может потребовать дополнительное переключение, а переключение на персонажа со статусом Крио заморозит героя.<br>


# === characters.#xingqiu.roles.#off-field_dps.notes ===

**О выборе оружия**

Место в рейтинге предполагает, что Восстановления энергии достаточно для постоянного дейтсвия Взрыва стихии. Обычно это 180-200% Восстановления для [Церемониального меча](#weapon:sacrificial-sword) до \[С6\] и 150-170% после. С другими оружиями нужно 240% и 190% Восстановления соответственно.

**[Драгоценный омут](#weapon:primordial-jade-cutter)**<br>
Лучшее оружие для Син Цю, если у него Восстановление энергии 240% без \[С6\] и 190% \[С6\].

**[Церемониальный меч](#weapon:sacrificial-sword)**<br>
Предполагается, что пассивка будет срабатывать вместе с каждым Элементальным навыком. Если навык нужно вызывать по откату, такое возможно только на \[Р3+\]. На \[Р1-2\] этот меч будет хуже [Кромсателя пиков](#weapon:summit-shaper).

**[Меч Фавония](#weapon:favonius-sword)**<br>
Хоть его урон и меньше, дополнительная энергия от пассивки меча может оказаться полезнее для некоторых команд.

**[Предвестник зари](#weapon:harbinger-of-dawn)**<br>
Хорошее оружие для Син Цю, если у него \[С6\] или второй Гидро персонаж в отряде для генерации энергии.

**О приоритете оружия на \[С6\]**
Когда у Син Цю \[С6\], [Драгоценный омут](#weapon:primordial-jade-cutter), [Рассекающий туман](#weapon:mistsplitter-reforged), [Харан гэппаку фуцу](#weapon:haran-geppaku-futsu), [Небесный меч](#weapon:skyward-blade), [Кромсатель пиков](#weapon:summit-shaper), и 
[Амэнома Кагэути](#weapon:amenoma-kageuchi), в этом порядке, имеют больший потенциал урона, чем [Церемониальный меч \[R3+\]](#weapon:sacrificial-sword).

**О наборе Артефактов**

**[Эмблема рассечённой судьбы (4)](#artifact:emblem-of-severed-fate)**<br>
При наличии \[С6\] у Син Цю, собранного в Церемонию древней знати (2), Сердце глубин (2) и Церемониальный меч, нет смысла целенаправленно добывать этот комплект.

**[Церемония древней знати (4)](#artifact:noblesse-oblige)**<br>
Дает всему отряду бонус атаки, можно использовать на Син Цю, если больше ни у кого в отряде нет этого комплекта.

**[Усмиряющий гром (4)](#artifact:thundersoother)**<br>
Этот комплект подходит только для отрядов, играющих через реакцию Заряжен. Он такой же, как Церемония древней знати (2) и Сердце глубин (2) по бонусу урона, но также увеличивает урон Элементального навыка, при условии, что на противнике наложено Электро. Не рекомендуется целенаправленно добывать этот комплект.

**[Заблудший в метели (4)](#artifact:blizzard-strayer)**<br>
Этот комплект подходит только для отрядов, играющих через Заморозку. Из-за бонуса на Криты, может увеличить урон Син Цю, если собрать артефакты на Крит. урон. Не смотря на то, что в таком случае этот комплект превосходит Эмблему рассечённой судьбы (4), не рекомендуется целенаправленно добывать этот комплект.

**[Изгнанник (2) Учёный (2)](#artifact:the-exile)**<br>
Этот комплект хороший вариант для начинающих игроков, так как он дает 40% Восстановления энергии. Обратите внимание, что в последствии комплект нужно сменить, потому что с ним Син Цю теряет в уроне.

**О приоритете основных характеристик**

В идеале, нужно брать часы на Силу атаки, когда собрано достаточно Восстановления энергии — 180-200% Церемониальным мечом, 240%+ без него.


# === characters.#xinyan.roles.#dps.name ===

ДД


# === characters.#xinyan.roles.#dps.tips ===

Элементальный навык Синь Янь накладывает Пиро статус на активного персонажа. Не важно как часто переключаться между героями, он наложит Пиро при каждом переключении. Этот позволяет снимать любой элементальный статус, который был наложен на персонажей.

Если статус не был снят, следует переключиться на этого персонажа еще раз. Пиро плохо снимает Гидро эффект, например, в Саду Сесилии нужно наложить на персонажа Пиро четыре раза, чтобы снять Гидро статус.


# === characters.#xinyan.roles.#dps.notes ===

**О выборе оружия**

[Меч драконьей кости](#weapon:serpent-spine)<br>
Стоит обратить внимание, что \[Р5\] **Меч драконьей кости** может превзойти всё пятизвездочное оружие с \[Р1\] при правильном использовании баффа

**О комплектах артефактов**

[Рыцарь крови (4)](#artifact:bloodstained-chivalry)<br>
Не смотря на то, что стиль игры за Синь Янь строится вокруг ее заряженных атак, полный комплект Рыцаря крови не рекомендуется, так как его бафф накладывается только после убийства.


# === characters.#xinyan.roles.#shield_support.name ===

Щитовик


# === characters.#xinyan.roles.#shield_support.subStats.advices.1.notes ===

крит. урон, если С2


# === characters.#xinyan.roles.#shield_support.subStats.notes ===

для щита приоритетнее Восстановление энергии, чем процент атаки


# === characters.#xinyan.roles.#shield_support.notes ===

**О комплектах артефактов**

[Церемония древней знати (4)](#artifact:noblesse-oblige)<br>
Этот комплект может быть использован для баффа Силы атаки всей пачки, если никто другой в отряде его не использует.

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith) и [Церемониальный двуручный меч](#weapon:sacrificial-greatsword)**<br>
Синь Янь как персонаж поддержки в основном используется для щита и/или урона от Взрыва стихии. Если использовать её как Физ саппорта, ей необходим пассивный талант на 60 уровне и \[С4\] для максимального увеличения способностей поддержки, что делает её довольно дорогим выбором для роли Физ саппорта. Однако, комплект Стойкость Миллелита даёт ей новую роль, не требующую такого высокого уровня созвездия.

**[Стойкость Миллелита](#artifact:tenacity-of-the-millelith)**<br>
Этот комплект даёт Синь Янь уникальную роль среди щитовиков. На данный момент, Синь Янь одна из немногих из щитовиков, кто может полностью использовать бонусы комплекта Стойкость Миллелита. Если максимизировать время активности щита третьего уровня Синь Янь, она сможет предоставить постоянный бафф на 30% стойкости щита и 20% бонуса урона отряду, пока пульсирования щита ударяют врага. Это означает, что **Церемониальный двуручный меч** и Синь Янь \[С2\] крайне важны, чтобы комплект идеально работал на ней. В противном случае, комплект не настолько хорош, и лучше собрать её в Ульт саппорта или щитовика.

На \[С2\] её Взрыв стихии имеет 100% Шанс крит. попадания, что позволяет собирать только Крит. урон, вместо обычного подхода со сбором 1:2 Шанс крит. попадания:Крит. урону, в случае сборки саппорта с физическим Взрывом стихии.

**В целом, не рекомендуется для накладывания Пиро, так как щит пульсирует только на третьем уровне, и не эффективен для взлома Крио щитов врагов.**


# === characters.#xinyan.roles.#sub-dps.name ===

Доп. ДД


# === characters.#xinyan.roles.#sub-dps.artifacts.notes ===

доп.ДД собирает статы как у глав.ДД.


# === characters.#xinyan.roles.#sub-dps.subStats.advices.1.notes ===

крит. урон, если С2


# === characters.#xinyan.roles.#sub-dps.subStats.notes ===

для щита приоритетнее Восстановление энергии, чем процент атаки


# === characters.#xinyan.roles.#sub-dps.notes ===

`replace-from:characters.#xinyan.roles.#shield_support.notes`


# === characters.#yae-miko.roles.#off-field_dps.name ===

Карманный ДД


# === characters.#yae-miko.roles.#off-field_dps.artifacts.notes ===

\~= [18% Силы атаки (2)](#artifact:18%-atk) / [Громогласный рёв ярости (2)](#artifact:thundering-fury) / [Эмблема рассечённой судьбы (2)](#artifact:emblem-of-severed-fate)\* \[Выберите два\]


# === characters.#yae-miko.roles.#off-field_dps.tips ===

Перед Взрывом стихии Яэ Мико нужно трижды использовать свой Элементальный навык, чтобы увеличить урон Взрыва.

**Комбо Взрыва стихии**<br>
Элементальный навык (ЭН)> ЭН > ЭН > Взрыв стихии > ЭН > ЭН > ЭН

Обратите внимание на то, что ни одна из способностей Яэ Мико не снапшотает.


# === characters.#yae-miko.roles.#off-field_dps.notes ===

**О выборе оружия**

**[Солнечная жемчужина](#weapon:solar-pearl)**<br>
Урон этого оружия сильно зависит от Ранга пробуждения и баффов атаки, например, баффа Беннета. Солнечная жемчужина может быть сравнима по урону с [Небесным атласом](#weapon:skyward-atlas) на \[Р5\] без баффов атаки, или наоборот, на низком Ранге пробуждения, но с баффами атаки, например, от Беннета. Следует не забывать использовать обычную атаку при переключении с Яэ и на Яэ, чтобы поддерживать бафф бонуса оружия.

**[Песнь странника](#weapon:the-widsith)**<br>
Несмотря на талант от возвышения Яэ на 60 уровне, бонус Мастерства стихий этого оружия плох для неё. Песнь странника будет хуже, чем [Солнечная жемчужина](#weapon:solar-pearl) с тем же Рангом пробуждения, и хуже, чем [Око клятвы](#weapon:oathsworn-eye), в случае баффа на Мастерство стихий. Если же использовать это оружие только с бонусами на урон, сбрасывая бонус на Мастерство стихий, то это оружие может превзойти пятизвездочное. В целом, рекомендуется использовать другое оружие для более стабильного нанесения урона.

**[Кольцо Хакусин](#weapon:hakushin-ring)**<br>
Пассивная способность этого оружия может баффать других членов отряда, что в некоторых отрядах может быть предпочтительнее, чем личный урон Яэ Мико. Обратите внимание на то, что пассивка не может быть активирована из кармана.

**О комплектах артефактов**

**[Эмблема рассечённой судьбы (4)](#artifact:emblem-of-severed-fate)**<br>
Этот комплект лучше всего подходит для ситуации, когда Взрыв стихии Яэ попадает одновременно по нескольким противникам. Также, этот комплект лучше подходит для игры без Райдэн в отряде, из-за большей необходимости в Восстановлении энергии.

**[18% Силы атаки (2)](#artifact:18%-atk) / [Громогласный рёв ярости (2)](#artifact:thundering-fury) / [Эмблема рассечённой судьбы (2)](#artifact:emblem-of-severed-fate)**<br>
Набор этих артефактов лучше подходит для ситуации, когда Яэ сражается с одним противником. Также, этот набор лучше подходит для игры вместе с Райдэн в отряде благодаря  меньшей необходимости в Восстановлении энергии.

**[Усмиряющий гром (4)](#artifact:thundersoother)**<br>
Если все время поддерживать действие бонуса комплекта, то это лучшие артефакты для Яэ. Лучше всего этот комплект подходит для Яэ в командах построенных через реакцию Заряжен.

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)**<br>
С этим комплектом Яэ теряет в личном уроне, но зато даёт  20% бонус Атаки всему отряду. Не рекомендуется добывать этот комплект специально для Яэ, и его использовать можно только если уже есть хорошие артефакты с этого подземелья.

**О приоритете талантов**

Обратите внимание на то, что Взрыв стихии наносит больше урона, чем Элементальный навык при битве с группой врагов.

Спасибо _Fob#6835_ за помощь в составлении [рейтинга артефактов и оружия (англ.)](https://docs.google.com/spreadsheets/d/1kDgkF3ZCa6MiQJLFzSXVBqbLrhe-0fNk/edit#gid=606412994)


# === characters.#yanfei.roles.#dps.name ===

ДД


# === characters.#yanfei.roles.#dps.tips ===

Пассивное умение 60 уровня Янь Фэй наносит второй удар после крита Заряженной атакой. Урон этого удара равен 80% Силы атаки и тоже может вызывать реакцию, благодаря собственному времени отката наложения Пиро. <br>


# === characters.#yanfei.roles.#dps.notes ===

Игра через Испарение — оптимальный вариант, и Син Цю подходит в команду для этого как никто другой. Три обычные атаки и затем одна Заряженная — лучший способ вызывать Испарение на Заряженных атаках. Главное, не забывать предварительно накладывать Гидро.

**О выборе оружия**

[Молитва святым ветрам](#weapon:lost-prayer-to-the-sacred-winds)<br>
Бафф Беннета или бафф Эпоса о драконоборцах плюс комплект Церемонии древней знати (4) выдвигает Молитву святым ветрам вперед Небесного атласа и Памяти о пыли.

[Истина кагура](#weapon:kaguras-verity)
С баффом Беннета или Эпоса о драконоборцах и баффом от полного комплекта Церемонии древней знати, Истина кагура превзойдет Небесный атлас и Память о пыли.

[Песнь странника](#weapon:the-widsith)
С сильным баффом Силы атаки, таким как Взрыв стихии Беннета, \[Р5\] Песнь странника превзойдет все \[Р1\] пятизвездочные оружия, кроме [Молитвы святым ветрам](#weapon:lost-prayer-to-the-sacred-winds).

[Истории Додоко](#weapon:dodoco-tales)<br>
На одинаковом Ранге пробуждения, Песнь странника лучше, чем Истории Додоко, но сравния \[Р5\] Истории Додоко и \[Р1-3\] Песнь странника, \[Р5\] Истории Додоко лучше. Обратите внимание, что бафф Беннета или бафф Эпоса о драконоборцах плюс бафф комплекта Церемония древней знати (4) может вывести Песнь странника и Солнечную жемчужину вперед Историй Додоко, даже при низком Ранге пробуждения.

**О наборе Артефактов**

[Воспоминания Симэнавы (4)](#artifact:shimenawas-reminiscence)<br>
Взрыв стихии Янь Фэй редко будет заряжен, из-за стоимости баффа полного комплекта в 15 энергии, зато она может дольше поддерживать **бафф четырех частей, что делает этот комплект хорошим вариантом.** Обратите внимание, что этот комплект теряет актуальность на больших созвездиях, так как она не сможет получить бонусы от \[С4\] без использования Взрыва стихии.

[Встречная комета (4)](#artifact:retracing-bolide)<br>
Использование Янь Фэй с щитовиком позволяет использовать комплект Встречная комета (4) как временный, пока не будет собран полный комплект Горящей алой ведьмы.

**О приоритетных допстатах в артефактах**

Восстановление энергии приоритетнее Мастерства стихий, из-за стоимости её Взрыва стихий в 80 энергии и её Элементального навыка, не эффективного в генерации частичек. Основным источником энергии для неё являются поверженные враги и Пиро батарейки, такие как Беннет, поэтому следует всегда собирать частицы самой Янь Фэй, так как неактивные персонажи получают только 0.6 энергии от собранных частичек.


# === characters.#yanfei.roles.#shield_support__[c4+_required].name ===

Щитовик \[С4+\]


# === characters.#yanfei.roles.#shield_support__[c4+_required].talents.notes ===

Для этой роли таланты Янь Фэй прокачивать необязательно.


# === characters.#yanfei.roles.#shield_support__[c4+_required].tips ===

**Восстановление энергии и оружие**

Прототип: Янтарь \[Р5\]: 210 Восстановления энергии<br>
Прототип: Янтарь \[Р1\] или Кодекс Фавония: 240 Восстановления энергии<br>
Эпос о драконоборцах: 250+ Восстановления энергии


# === characters.#yanfei.roles.#shield_support__[c4+_required].notes ===

Этот билд посвящен использованию Янь Фэй как главщитовика благодаря её четвертому созвездию. Из-за необходимости использовать Взрыв стихии по откату, у этого билда высокие требования к Восстановлению энергии.


# === characters.#yoimiya.credits ===

[Подробный разбор](https://keqingmains.com/yoimiya/) от Ёимия мейнеров и ККМ (англ.)


# === characters.#yoimiya.roles.#dps.name ===

ДД


# === characters.#yoimiya.roles.#dps.notes ===

**О выборе оружия**

**[Ржавый лук](#weapon:rust)**<br>
На более высоких рангах пробуждения, это оружие может превзойти Лук Амоса и Небесное крыло с низкими рангами пробуждения.

**[Хамаюми](#weapon:hamayumi)**<br>
Это оружие плохо сочетается с комплектом Воспоминания Симэнавы (4) из-за его бонуса.

**[Прототип: Полумесяц](#weapon:prototype-crescent)**<br>
Обратите внимание: для активации пассивки и полноценного использования этого оружия нужно попадать прицельным выстрелом в слабое место противника.

**[Рогатка](#weapon:slingshot)**<br>
С мощным баффом на Силу атаки это оружие сравнимо с Ржавым луком на \[Р1\], с учетом радиуса активации баффа Рогатки и хорошим аптаймом баффа на Силу атаки.

**Об Артефактах**

**[Горящая алая ведьма (2)](#artifact:crimson-witch-of-flames) плюс комплект на [18% Силы атаки (2)](#artifact:18%-atk)**<br>
Лучший универсальный комплект для Ёимии: сопоставим с остальными по собственному урону, при этом работает в любой ситуации.

**[Воспоминания Симэнавы](#artifact:shimenawas-reminiscence)**<br>
Дает Ёимии больше урона, но жертвует аптаймом её Взрыва стихий, что приводит к понижению общего урона отряда, так как теряется возможность полностью использовать её пассивный талант 60 уровня.

**[Горящая алая ведьма (4)](#artifact:crimson-witch-of-flames)**<br>
Стандартный комплект артефактов для сфокусированных на реакции Пар Пиро ДД, **превосходит другие комплекты в отрядах, сфокусированных на реакции.**

**[Ступающий по лаве (4)](#artifact:lavawalker)**<br>
Хороший вариант для моно Пиро отряда, где Ёимия может держать 100% аптайм бонуса комплекта.

**О главстатах**

**Мастерство стихий**<br>
Хороший вариант для Ёимии — вызывать Испарение или Перегрузку. Если в команде есть Беннетт, часы на Мастерство стихий будут немного полезнее, чем на Силу атаки. Но рекомендуется использовать те, у которых лучше допстаты.

**О приоритетных талантах**

Элементальный навык скейлится очень слабо, поэтому он имеет самый низкий приоритет. Обычная отака имеет первоочередный приоритет потому, что это основной источник урона Ёимии.


# === characters.#yun-jin.roles.#support.name ===

Саппорт


# === characters.#yun-jin.roles.#support.notes ===

Юнь Цзинь  — это героиня поддержки для авто-атакеров, которая наносит урон своим Взрывом стихии. Взрыв стихии **Юнь Цзинь даёт ближайшим членам отряда 30 зарядов бонусного урона обычных атак, который зависит от её текущей защиты.** Это значит, что внешние баффы Защиты, такие как Элементальный навык и/или Взрыв стихии Горо не дают много Юнь Цзинь, из-за того, что баффают только активного персонажа и не баффают карманных.

**О выборе оружия**

[Копьё Фавония](#weapon:favonius-lance)<br>
С этим оружием стоит раздобыть немного Шанса крит. попадания, чтобы активировать пассивную способность копья.

[Крест-копьё Китаин](#weapon:kitain-cross-spear)<br>
Хоть главстат Мастерства стихий и бесполезен для Юнь Цзинь, его пассивка даёт много Восстановления энергии, особенно на более высоком ранге пробуждения. Это может снизить её потребность в Восстановлении энергии на 20% на \[Р1\] на 70% на \[Р5\]. Следует обратить внимание, что она должна использовать Взрыв стихии перед Элементальным навыком, так как пассивка забирает 3 единицы энергии при использовании Элементального навыка.

**О приоритетах в главстатах и допстатах при выборе артефактов**

Шанс крит. попадания стоит рассматривать только когда Юнь Цзинь экипирована Копьём Фавония, для надежного срабатывания пассивки.


# === characters.#zhongli.roles.#physical_dps.name ===

Физ. ДД


# === characters.#zhongli.roles.#physical_dps.notes ===

**Оптимальная последовательность физ. атак**<br>
Обычная атака до пинка по копью > Прыжок или сприт > повторить


# === characters.#zhongli.roles.#shield_support.name ===

Щитовик


# === characters.#zhongli.roles.#shield_support.notes ===

Довольно очевидный стиль игры, нужно собирать как можно больше ХП. Выбрать оружие с бонусом ХП, например Чёрную кисть, и комплект артефактов Стойкость Миллелита (4). В этом билде Взрыв стихии — потеря урона отряда, поэтому он используется только для контроля противников.

С баффом снижающим получаемый урон можно сделать щит Чжуна ли практически неразрушаемым. Такой бафф можно получить от Элементального навыка и Взрыва стихии Син Цю и от Взрыва стихии Бэй Доу.

**О выборе оружия**

[Копьё Фавония](#weapon:favonius-lance)<br>
Это оружие предпочтительнее [Чёрной кисти](#weapon:black-tassel), если отряду нужна энергия. Но в таком случае нужно будет собирать Шанс крит. попадания, чтобы активировать бонус копья.


# === characters.#zhongli.roles.#burst_support.name ===

Ульт-саппорт


# === characters.#zhongli.roles.#burst_support.notes ===

Чжун Ли в отряде используется для хорошего щита (скейлится от ХП) и нанесения Гео урона Взрывом стихии, так как он откатывается за 12 секунд, стоит всего 40 энергии, и тоже скейлится от ХП после 60 уровня.

**О главстатах артефактов**

Урон будет больше с часами на бонус атаки, прочность щита будет выше с часами на бонус ХП.

**Regarding Artifact Sets**

**[Церемония древней знати (2)](#artifact:noblesse-oblige) + [Архаичный камень (2)](#artifact:archaic-petra)**<br>
Если много восстановления энергии не требуется, этот набор будет полезнее, чем [Эмблема рассечённой судьбы (4)](#artifact:emblem-of-severed-fate).

**[Церемония древней знати (4)](#artifact:noblesse-oblige)**<br>
Добавляет полезности Взрыву стихии Чжуна Ли. Но не стоит использовать этот набор, если у кого-то в команде он уже есть.

**[Стойкость Миллелита (4)](#artifact:tenacity-of-the-millelith)**<br>
Добавлет полезности Элементальному навыку Чжуна Ли (если навык попадает по врагу). А ещё этот набор услиливает щиты.