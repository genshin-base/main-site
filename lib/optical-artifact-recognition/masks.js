 ...
 * @param {number} mult
 */
export function maskHLinesSobelD1(mask, mult) {
	const { width, height } = mask.rect.toSize()
	const { iFrom, jFrom } = mask.rect
	const { buf, stride } = mask
	if (width <= 1) return
	for (let j = 0; j < height; j++) {
		let prev = buf[iFrom + stride * (j + jFrom)]
		let cur = prev
		for (let i = 0; i < width; i++) {
			const next = i + 1 + iFrom >= width ? cur : buf[i + 1 + iFrom + stride * (j + jFrom)] //TODO: speed: move check from loop
			buf[i + iFrom + stride * (j + jFrom)] = Math.abs(prev - next) * mult
			prev = cur
			cur = next
		}
	}
 ...
 * @param {number} mult
 */
export function maskVLinesSobelD1(mask, mult) {
	const { width, height } = mask.rect.toSize()
	const { iFrom, jFrom } = mask.rect
	const { buf, stride } = mask
	if (width <= 1) return
	for (let i = 0; i < width; i++) {
		let prev = buf[iFrom + i + stride * jFrom]
		let cur = prev
		for (let j = 0; j < height; j++) {
			const next = j + 1 + jFrom >= height ? cur : buf[i + iFrom + stride * (j + 1 + jFrom)] //TODO: speed: move check from loop
			buf[i + iFrom + stride * (j + jFrom)] = Math.abs(prev - next) * mult
			prev = cur
			cur = next
		}
