 ...
 * @param {string} tokenFilePath
 * @param {string} spreadsheetId
 * @param {SpreadsheetsRequest} request
 * @returns {Promise<void>}
 */
export async function updateSpreadsheet(authFilePath, tokenFilePath, spreadsheetId, request) {
	const { accessToken } = await updateTokenIfNeed(authFilePath, tokenFilePath)

	/** @type {SpreadsheetBatchUpdateRequest} */
	const params = { requests: [request] }
