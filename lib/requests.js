	...
	return new Promise((resolve, reject) => {
		const req = httpsRequest(url, { method: 'GET' }, response => {
			if (canRedirect && response.statusCode && ((response.statusCode / 100) | 0) === 3) {
				...
				console.log('FILE: redirecting to: ' + location)
				getFile(location, null, fpath, false).then(resolve).catch(reject)
				return
			}

			if (response.statusCode !== 200)
				throw new Error(`unexpected status ${response.statusCode} in response to ${url}`)
			...
			response.on('data', chunk => {
				...
				if (now - prevTime > 1000) {
					...
					process.stdout.write(
						`FILE: ${mib} MiB` + (isNaN(size) ? '' : `${((total * 100) / size) | 0}%`) + '\r',
					)
					prevTime = now
				}
			})
...
export async function getFileCached(url, data, fpath, canRedirect, cacheDurationMS) {
	...
	if (!stat || isOld) {
		console.log(`FILE: ${relativeToCwd(fpath)} is ${isOld ? 'old' : 'missing'}, downloading`)
