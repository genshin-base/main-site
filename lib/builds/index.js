...
import { extractArtifactRefs, json_extractArtifactsInfo } from './artifacts.js'
...
export async function extractBuildsFromODS(odsFPath, jsonFPath, fixes) {
	...
	const { artifacts } = extractItemsInfo(jsonData)
...
function extractItemsInfo(jsonData) {
	return {
 ...
 * @returns {Promise<BuildInfo>}
 */
async function processSpreadsheet(root, artifacts, fixes) {
	...
	for (const table of spreadsheet.children) {
		...
		if (ELEMENT_NAMES.has(tableName)) elementMap[tableName] = processElementTable(table, artifacts, fixes)
	...
	for (const elem of Object.values(elementMap))
		for (const char of elem) {
			...
			for (const r of char.roles) {
				add(r.weapon, r.mainStats, r.subStats, r.talent, r.tips)
			}
		}
 ...
 * @param {import('./utils.js').BuildsExtractionFixes} fixes
 */
function processElementTable(table, artifacts, fixes) {
