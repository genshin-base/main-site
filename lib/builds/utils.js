/**
 * @typedef {{
 *   weapons: {col:'name'|'mainStat'|'subStat'|'passive', replace:RegExp, with:string, _used?:boolean}[],
...
export function clearFixesUsage(fixes) {
	for (const attr of [fixes.weapons, fixes.charactersArtifactsMatch, fixes.charactersWeaponsMatch])
		attr.forEach(clear)
}
/**
 * @param {BuildsExtractionFixes} fixes
