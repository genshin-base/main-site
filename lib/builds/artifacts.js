...
/** @typedef {{title:string, sets:ArtifactRefSet[], notes:string, seeCharNotes:boolean}} ArtifactRefGroup */

/** @typedef {{groups:ArtifactRefGroup[]}} ArtifactRefs */
...
export function json_extractArtifactsInfo(artsSheet) {
	/**
	 * @typedef {{
	 *   nameCol: number,
	...
	for (const { values: cells = [] } of artsSheet.data[0].rowData) {
		...
		if (curStars) {
			let _col
			if ((_col = json_findCellIndex(cells, /^artifact\s+set$/)) !== -1) {
				if (_col !== -1) {
					artSetInfo = { nameCol: _col, ext: null }
					if ((_col = json_findCellIndex(cells, /^1 set bonuses$/)) !== -1)
						artSetInfo.ext = { oneCol: _col }
				}
			} else if (artSetInfo && !artSetInfo.ext) {
				artSetInfo.ext = {
					twoCol: json_mustFindCellIndex(cells, /^2\Dpieces$/),
					fourCol: json_mustFindCellIndex(cells, /^4\Dpieces$/),
				}
