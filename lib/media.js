 ...
 */
export async function resize(inFPath, outFPath, size) {
	return run('magick', [inFPath, '-resize', size, outFPath])
}
...
export async function pngquant(inFPath, outFPath, speed = 1) {
	return run('pngquant', ['--force', '--strip', '--speed=' + speed, '--output', outFPath, inFPath])
}
...
export async function optipng(inFPath, outFPath, level = 3) {
	...
	return run('optipng', ['-clobber', '-quiet', '-o' + level, inFPath, '-out', outFPath])
}
 ...
 * @param {string} inFPath
 * @param {string} outFPath
 * @param  {...((inFPath:string, outFPath:string) => Promise<unknown>)} chain
 */
