...
/**
 * Обёртка вокруг `obj[key] ?? defaultVal`, чтоб тайпчекер со своим
 * `Element implicitly has an 'any' type` был доволен.
 * @template {string} TKey
 * @template TVal
 * @param {Partial<Record<TKey, TVal>>} obj
 * @param {string} key
 * @param {TVal} defaultVal
 * @returns {TVal}
 */
export function objGet(obj, key, defaultVal) {
