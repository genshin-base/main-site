...
import { getCharacterBuildFormattedBlocks } from './index.js'
...
/**
 * @param {import('./types').CharacterBuildInfo<'monolang'>} character
 * @param {import('#lib/parsing').ArtifcatSetGroupCodes|null} artGroupCodes
 */
export function getCharacterArtifactCodes(character, artGroupCodes) {
	...
	for (const role of character.roles)
		for (const set of role.artifacts.sets) {
			for (const code of getArtCodesInSet(set.arts, artGroupCodes)) codes.add(code)
		}
	for (const node of getCharacterTextNodes(character))
		if ('artifact' in node) {
			codes.add(node.code)
 ...
 */
export function getCharacterRecommendedArtifactCodes(character, artGroupCodes) {
	const codes = /**@type {Set<string>}*/ (new Set())
	...
	for (const role of character.roles) {
		...
		if (set) for (const code of getArtCodesInSet(set.arts, artGroupCodes)) codes.add(code)
	}
...
function* getArtCodesInSet(arts, artGroupCodes) {
	// getArtifactSpecialGroupCodes
	yield* (function* iter(node) {
		if ('code' in node) {
			yield node.code
			if (artGroupCodes && node.code in artGroupCodes) {
				for (const code of artGroupCodes[node.code]) yield code
			}
