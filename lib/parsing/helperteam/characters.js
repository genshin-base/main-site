...
export function* getCharacterArtifactCodes(character) {
	for (const role of character.roles)
		for (const set of role.artifacts.sets) yield* getArtCodesInSet(set.arts)
}
/**
...
export function* getCharacterRecommendedArtifactCodes(character) {
	for (const role of character.roles) {
		...
		if (set) yield* getArtCodesInSet(set.arts)
	}
}
/**
 * @param {import('./artifacts').ArtifactRef | import('./artifacts').ArtifactRefNode} arts
 * @returns {Generator<string, void, void>}
 */
function* getArtCodesInSet(arts) {
