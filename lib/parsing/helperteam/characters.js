import { ART_GROUP_18_ATK_CODE, ART_GROUP_20_ER_CODE, getCharacterCodeFromName } from '#lib/genshin.js'
...
export function* getCharacterArtifactCodes(character, artGroupCodes) {
	for (const role of character.roles)
		for (const set of role.artifacts.sets) {
			yield* getArtCodesInSet(set.arts, artGroupCodes)
		}
	for (const node of getCharacterTextNodes(character))
		if ('artifact' in node) {
			yield node.code
...
export function* getCharacterRecommendedArtifactCodes(character) {
	for (const role of character.roles) {
		...
		if (set) yield* getArtCodesInSet(set.arts, null)
	}
}
/**
