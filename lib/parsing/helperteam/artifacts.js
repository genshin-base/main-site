import { ART_GROUP_18_ATK_CODE, ART_GROUP_20_ER_CODE, getArtifactCodeFromName } from '#lib/genshin.js'
import { warn } from '#lib/utils.js'
import { getCharacterArtifactCodes } from './characters.js'
...
/** @typedef {import('./text').OneOrMoreTextNodes} OneOrMoreTextNodes */
...
/** @typedef {ArtifactInfo & {name:string, recommendedTo:string[]}} ArtifactFullInfo */
...
export function json_extractArtifactsInfo(sheet, knownCodes) {
	...
	for (const { values: cells = [] } of sheet.data[0].rowData) {
		for (let i = 0; i < cells.length; i++) {
			const cell = cells[i]
			const m = json_getText(cell)
				.trim()
				.match(/^(\d)\s*[⭐✩]\s*artifact 1?\s*sets/i)
			if (m) artSetInfo = null
		}
...
export function extractArtifactMainStatAdvices(lines, characterCode, fixes) {
	...
	for (let line of lines) {
		...
		if ((m = line.match(/^(circlet|goblet|sands)\s*-(.*)$/i)) !== null) {
			...
			transferLongNotes(advices, { b: type + ': ' }, advice)
			advices[type] = advice
		} else {
...
/**
 * @param {{notes:OneOrMoreTextNodes|null}} destObj
 * @param {import('./text').OneOrMoreInlineTextNodes} prefix
 * @param {{notes:OneOrMoreTextNodes|null}} srcObj
 */
 ...
 * @param {import('#lib/parsing').Code2ArtifactData} code2artifactData
 * @param {import('./characters').CharacterBuildInfo[]} characters
 * @param {string} lang
 * @returns {ArtifactFullInfo}
 */
export function makeArtifactFullInfo(artifact, code2artifactData, characters, lang) {
	...
	if (data) {
		...
		warn(`can not get artifact full info: '${artifact.code}' not found`)
	}
	const recommendedTo = new Set() //traveler встречается несколько раз
	for (const character of characters)
		for (const code of getCharacterArtifactCodes(character))
			if (code === artifact.code) {
				recommendedTo.add(character.code)
				break
			}
	return Object.assign({ name, recommendedTo: Array.from(recommendedTo) }, artifact)
}
