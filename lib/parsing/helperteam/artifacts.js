...
export function json_extractArtifactsInfo(sheet, knownCodes) {
	...
	const artifacts = /**@type {import('./types').ArtifactInfo<'monolang'>[]}*/ ([])
	...
	for (const { values: cells = [] } of sheet.data[0].rowData) {
		...
		if ((_col = json_findCellIndex(cells, /^artifact\s+set$/i)) !== -1) {
			...
			if (name && (sets[1] || sets[2])) {
				...
				const code = getCodeChecked(getArtifactCodeFromName, knownCodes, name, 'artifacts sheet')
		...
		if (m) {
			...
			while (true) {
				...
				if (/\+?18%atkset/i.test(name.replace(/\s/g, ''))) {
					...
					code = getCodeChecked(getArtifactCodeFromName, codeSearcher, name, logPrefix)
				}
