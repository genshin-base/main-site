...
/**
 * @param {OneOrMoreTextNodes} nodes
 * @param {(text:string) => string} func
 * @returns {OneOrMoreTextNodes}
 */
export function updateTextNodesText(nodes, func) {
	if (Array.isArray(nodes)) {
		for (let i = 0; i < nodes.length; i++) {
			const node = nodes[i]
			if (typeof node === 'string') {
				...
				updateTextNodesText(node, func)
			}
		}
		return nodes
	}

	if (typeof nodes !== 'string' && 'p' in nodes) {
		const pContent = nodes.p
		if (Array.isArray(pContent)) {
			for (let i = 0; i < pContent.length; i++) {
				pContent[i] = updateInlineTextNodesText(pContent[i], func)
			}
			return nodes
		} else {
			nodes.p = updateInlineTextNodesText(pContent, func)
			return nodes
		}
	}

	return updateInlineTextNodesText(nodes, func)
}
/**
 * @param {TextNodeInline} node
 * @param {(text:string) => string} func
 * @returns {TextNodeInline}
 */
function updateInlineTextNodesText(node, func) {
	if (typeof node === 'string') return func(node)

	if ('b' in node) node.b = updateInlineTextNodesText(node.b, func)
	else if ('i' in node) node.i = updateInlineTextNodesText(node.i, func)
	else if ('s' in node) node.s = updateInlineTextNodesText(node.s, func)
	else if ('a' in node) node.a = updateInlineTextNodesText(node.a, func)
	else if ('weapon' in node) node.weapon = updateInlineTextNodesText(node.weapon, func)
	else if ('artifact' in node) node.artifact = updateInlineTextNodesText(node.artifact, func)
	else if ('item' in node) node.item = updateInlineTextNodesText(node.item, func)
	else mustBeNever(node)
