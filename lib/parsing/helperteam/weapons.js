 ...
 *   passiveStat: import('./text').CompactTextParagraphs,
 * }} WeaponInfo
 */
 ...
 *   domain: {name:string, location:[x:number, y:number]},
 * }} WeaponFullInfo
 */
...
export function makeWeaponFullInfo(weapon, code2weaponData, code2domainData, lang) {
	...
	let domain = { name: '', location: /** @type {[number,number]} */ ([0, 0]) }
	...
	if (data) {
		...
		warn(`weapon '${weapon.code}': weapon full info not found, using default name and rarity`)
	}

	const domainData =
		data &&
		Object.values(code2domainData).find(x =>
			x.weaponMaterialGroupCodes.includes(data.primaryMaterialGroupCode),
		)
	if (domainData) {
		domain.name = getLangValue(domainData.name, lang, '', 'name', domainData.code)
		domainData.location = domainData.location
	} else {
		warn(`weapon '${weapon.code}': domain not found, using default location`)
	}

	return Object.assign({ name, rarity, domain }, weapon)
}
