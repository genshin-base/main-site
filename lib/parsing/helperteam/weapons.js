 ...
 * @param {import('#lib/parsing').WeaponsInfo} weaponInfos
 * @param {string} lang
 * @returns {WeaponFullInfo}
 */
export function makeWeaponFullInfo(weapon, weaponInfos, lang) {
	let name = weapon.code
	let rarity = /** @type {import('#lib/genshin').GI_RarityCode} */ (1)
	const info = weaponInfos[weapon.code]
	if (info) {
		name = getLangValue(info.name, lang, weapon.code, 'name', weapon.code)
		rarity = info.rarity
	} else {
		warn(`can not make weapon full info: '${weapon.code}' not found`)
	}
