...
const TEXT_FORMAT_USED_ATTRS = new Set(['link', 'bold', 'italic', 'strikethrough'])
const TEXT_FORMAT_BOOL_ATTRS = new Set(['bold', 'italic', 'strikethrough'])
...
export function json_extractText(cell, startOffset, endOffset, preprocess, mapInlineNodeFunc) {
	...
	const mapInlineNode = mapInlineNodeFunc ?? (x => x)
	...
	// выравнивание ссылок должно идти до оптимизации форматирования:
	// последнее игнорирует подчёркивания, может растянуть их до конца текста,
	// а ссылка тогда рястянется вслед за подчёркиванием.
	...
	for (let i = 0, textOffset = startOffset; i < pTexts.length; i += 2) {
		...
		if (pFormatRuns.length > 0) {
			...
			children = pFormatRuns.map((run, i, runs) => {
				...
				let node = mapInlineNode(nodeText)
				if (href) node = mapInlineNode({ a: node, href })
				if (format.bold) node = mapInlineNode({ b: node })
				if (format.italic) node = mapInlineNode({ i: node })
				if (format.strikethrough) node = mapInlineNode({ s: node })
				return node
			})
		} else {
			children = [pText]
...
function json_alignLinksWithWords(text, formatRuns) {
	...
	json_realignFormatRuns(
		...
		(run, i, start, end) => {
			...
			// В таблицах всетрчается странное форматирование:
			// часть текста рядом со ссылкой подчёркнута, но ссылкой не является.
			...
			if ((m = text.slice(0, start).match(/\w+$/)) !== null) {
				if (i === 0 || !formatRuns[i - 1].format.link) {
					const shift = m[0].length
			...
			if ((m = text.slice(end).match(/^[\w.]+/)) !== null) {
				if (i === formatRuns.length - 1 || !formatRuns[i + 1].format.link) {
					const shift = m[0].length
					const prevEnd = end
					end += shift
...
function json_alignStylesWithPunctuation(text, formatRuns) {
	json_realignFormatRuns(
		...
		(run, i, start, end) => {
			...
			let m
			// actually aligning with punctuation
