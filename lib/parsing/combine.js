...
/**
 * @param {import('#lib/parsing').Code2EnemyData} code2enemy
 * @param {CombineFixes} fixes
 * @returns {Map<string,string>}
 */
export function mergeSimilarEnemies(code2enemy, fixes) {
	/** @type {Map<string, {name:Record<string,string>, enemies:import('#lib/parsing').EnemyData[]}>} */
	const drop2group = new Map()
	for (const enemy of Object.values(code2enemy)) {
		...
		const group = drop2group.get(key)
		if (group) group.enemies.push(enemy)
		else drop2group.set(key, { enemies: [enemy], name: manualGroup?.name ?? {} })
	}

	const imgCode2groupCode = /**@type {Map<string,string>}*/ (new Map())
	for (const group of drop2group.values()) {
		if (group.enemies.length > 1) {
			...
			for (const enemy of group.enemies) delete code2enemy[enemy.code]

			code2enemy[code] = { code, name, drop: group.enemies[0].drop, locations: [] }

			imgCode2groupCode[group.enemies.map(x => x.code).sort()[0]] = code
		}
	}
	return imgCode2groupCode
}
