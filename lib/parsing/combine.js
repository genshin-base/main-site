 ...
 */
export function makeWeaponsFullInfo(weaponInfos, code2weapon, code2domain, code2item, characters, lang) {
	const fullWeapons = weaponInfos.map(x => makeWeaponFullInfo(x, code2weapon, characters, lang))
	const rels = {
		items: /**@type {Map<string, ItemShortInfo>}*/ (new Map()),
		domains: /**@type {Map<string, DomainShortInfo>}*/ (new Map()),
	}
...
export function makeMaterialsTimetable(
				...
				if (characterCodes.size > 0 || usedForWeapon) {
					...
					if (characterCodes.size > 0)
						dayTimetable.characterAscensions.push({
							itemCode,
							characterCodes: [...characterCodes].sort(),
						})
	...
	return /**@type {MaterialsTimetableWithRelated}*/ {
		...
		items: [...items.values()].sort((a, b) => a.code.localeCompare(b.code)),
	}
}
 ...
 */
function makeWeaponFullInfo(weapon, code2weaponData, charactersInfo, lang) {
...
function makeItemShortInfo(code, code2item, obtainSources, lang) {
	...
	return { code, name, obtainSources, ancestryCodes, locations }
}
