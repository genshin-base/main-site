import { encodeLocationsChecked } from '#lib/genshin.js'
...
function makeItemShortInfo(code, code2item, obtainSources, lang) {
	...
	let curItem = item
	while (curItem) {
		const isCraftedFrom3 = curItem.craftedFrom.length === 1 && curItem.craftedFrom[0].count === 3
		if (!isCraftedFrom3) break
		const code = curItem.craftedFrom[0].code
		ancestryCodes.push(code)
		curItem = code2item[code]
	}
