import { encodeLocations } from '#lib/genshin.js'
 ...
 *   code: string,
 *   name: string,
 *   locations: "external" | [x:number, y:number][],
 * }} EnemyShortInfo
 */
...
function extractItemsLocations(items) {
	...
	for (const item of items) {
		if (item.locations.length > 3 && item.locations !== 'external') {
			code2locs[item.code] = encodeLocations(item.locations, 2)
...
function makeItemShortInfo(code, code2item, obtainSources, lang) {
	...
	const isCraftedFrom3 = item && item.craftedFrom.length === 1 && item.craftedFrom[0].count === 3
	const craftedFrom3Code = isCraftedFrom3 ? item.craftedFrom[0].code : null

	return { code, name, obtainSources, craftedFrom3Code, locations }
}
