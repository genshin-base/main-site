import {
	...
	encodeLocationsChecked,
	GI_REGION_CODES,
	GI_ROTATION_WEEKDAY_CODES,
} from '#lib/genshin.js'
...
export function makeMaterialsTimetable(
	...
	for (const weekday of GI_ROTATION_WEEKDAY_CODES) {
		...
		sortItems(dayTimetable.characterAscensions, x => x.itemCode, code2domain)
		sortItems(dayTimetable.weaponMaterialCodes, code => code, code2domain)
	}
	...
	return /**@type {MaterialsTimetableWithRelated}*/ {
		...
		items: sortItems([...items.values()], item => item.code, code2domain),
	}
}
...
function makeWeaponFullInfo(weapon, code2weaponData, code2character, charactersInfo, lang) {
	...
	return Object.assign({ name, rarity, obtainSources, materialCodes, recommendedTo }, weapon)
}
...
/**
 * @param {import('#lib/parsing').CharacterData} a
 * @param {import('#lib/parsing').CharacterData} b
 * @returns
 */
function compareCharacters(a, b) {
	const dv = compareReleaseVersions(a.releaseVersion, b.releaseVersion)
	if (dv !== 0) return dv
	const aCode = a.code.endsWith('-traveler') ? 'traveler' : a.code
	const bCode = b.code.endsWith('-traveler') ? 'traveler' : b.code
 ...
 * @param {import('#lib/parsing').Code2CharacterData} code2character
 */
function sortCharacterCodes(codes, code2character) {
 ...
 * @returns {T[]}
 */
function sortCharacters(characters, codeFunc, code2character) {
 ...
 * @return {T[]}
 */
function sortItems(items, codeFunc, code2domain) {
	...
	return items.sort((a, b) => {
		...
		return (
			GI_REGION_CODES.indexOf(aRegion) - GI_REGION_CODES.indexOf(bRegion) || aCode.localeCompare(bCode)
		)
	})
}
