...
import { buildRoleConvertLangMode } from './helperteam/index.js'
...
/**
 * @typedef {import('./helperteam/types').ArtifactInfo<'monolang'> & {
 *   name: string,
 *   rarity: import('#lib/genshin').GI_RarityCode,
...
/**
 * @typedef {import('./helperteam/types').WeaponInfo<'monolang'> & {
 *   name: string,
 *   rarity: import('#lib/genshin').GI_RarityCode,
...
export function makeCharacterFullInfo(
	...
	const artifactCodes = new Set(getCharacterArtifactCodes(characterInfo, true))
	...
	const characterExt = {
		...
		roles: characterInfo.roles
			.map(role =>
				buildRoleConvertLangMode(role, 'monolang', vals =>
					getLangValueUnlessBlank(vals, lang, null, 'attr', `${characterInfo.code}:${role.code}`),
				),
			)
			.sort((a, b) => +b.isRecommended - +a.isRecommended),
		credits: getLangValueUnlessBlank(characterInfo.credits, lang, null, 'credits', characterInfo.code),
	}
...
/**
 * @param {import('./helperteam/types').ArtifactInfo<'multilang'>[]} artifactInfos
...
export function makeArtifactsFullInfo(
	...
	const fullArtifacts = artifactInfos.map(x =>
		makeArtifactFullInfo(x, code2artifact, code2character, code2domain, code2enemy, characters, lang),
	)
...
/**
 * @param {import('./helperteam/types').WeaponInfo<'multilang'>[]} weaponInfos
 ...
 */
export function makeWeaponsFullInfo(
	weaponInfos,
	code2weapon,
	code2character,
	code2domain,
	...
	const fullWeapons = weaponInfos.map(x =>
		makeWeaponFullInfo(x, code2weapon, code2character, characters, lang),
	)
	const rels = {
		items: /**@type {Map<string, ItemShortInfo>}*/ (new Map()),
		domains: /**@type {Map<string, DomainShortInfo>}*/ (new Map()),
...
/**
 * @param {import('./helperteam/types').ArtifactInfo<'multilang'>} artifact
 * @param {import('#lib/parsing').Code2CharacterData} code2character
 * @param {import('#lib/parsing').Code2ArtifactSetData} code2artifact
 * @param {import('#lib/parsing').Code2DomainData} code2domain
 * @param {import('#lib/parsing').Code2EnemyData} code2enemy
 * @param {import('./helperteam/types').CharacterBuildInfo<'multilang'>[]} charactersInfo
 * @param {string} lang
...
function makeArtifactFullInfo(
	...
	for (const character of charactersInfo)
		for (const code of getCharacterRecommendedArtifactCodes(character))
...
/**
 * @param {import('./helperteam/types').WeaponInfo<'multilang'>} weapon
 ...
 */
function makeWeaponFullInfo(weapon, code2weaponData, code2character, charactersInfo, lang) {
	...
	const recommendedTo = []
	for (const character of charactersInfo)
		for (const code of getCharacterRecommendedWeaponCodes(character, code2weaponData))
			if (code === weapon.code) {
				recommendedTo.push(character.code)
				break
			}
	...
	return {
		...
		name: data ? getLangValue(data.name, lang, weapon.code, 'name', weapon.code) : weapon.code,
		rarity: data ? data.rarity : 5,
		obtainSources: data?.obtainSources ?? [],
		materialCodes: data?.materialCodes ?? [],
		recommendedTo,
		mainStat: weapon.mainStat,
		subStat: weapon.subStat,
		passiveStat: getLangValue(weapon.passiveStat, lang, '', 'passive-stat', 'weapon'),
	}
}
