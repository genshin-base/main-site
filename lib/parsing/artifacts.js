import { addTextAsStringOrP, parseNotesLine } from './common.js'
...
/** @typedef {{arts:ArtifactRef|ArtifactRefNode} & BottomNotes} ArtifactRefSet */

/** @typedef {{sets:ArtifactRefSet[]} & BottomNotes} ArtifactRefs */
		...
		if (m) {
			...
			if (artSetRef.arts === DUMMY_ART_REF) {
				...
				if (artRefsGroups.length === 0)
					artRefsGroups.push({ title: null, refs: { sets: [], notes: null, seeCharNotes: false } })
				artRefsGroups.at(-1)?.refs.sets.push(artSetRef)
			}
		} else {
			if (line.endsWith(':')) {
				const title = line.slice(0, -1).trim()
				artRefsGroups.push({ title, refs: { sets: [], notes: null, seeCharNotes: false } })
			} else {
				const lastGroup = artRefsGroups.at(-1)
				if (lastGroup) {
					parseNotesLine(line, lastGroup.refs, rem =>
