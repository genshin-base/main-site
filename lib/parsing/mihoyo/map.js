 ...
 */
export async function applyItemsLocations(cacheDir, code2enemy, code2group, code2item) {
...

	{
		...
		for (const label of data.data.label_list) {
			...
			let code = getEnemyCodeFromName(label.name)
			...
			if (code in code2group) code2group[code].locations.push(...getPointsLocations(points))
			if (code in code2item) code2item[code].locations.push(...getPointsLocations(points))
		}
	}
