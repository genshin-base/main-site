...
/**
 * @typedef {{
 *   character: import('./characters').CharacterBuildInfo,
 *   artifacts: ArtifactInfo[],
 *   weapons: WeaponInfo[],
 * }} CharacterFullInfo
 */
 ...
 */
export function makeCharacterBuildInfo(build, character) {
	const weaponCodes = new Set()
	const artifactCodes = new Set()
	for (const role of character.roles) {
		for (const ref of role.weapons.refs) for (const similar of ref.similar) weaponCodes.add(similar.code)
