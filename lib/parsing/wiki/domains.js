...
import { getTextContent, searchNodeWithClass, searchNodeWithTag } from '#lib/xml.js'
import { mustFindCellIndex } from '../honeyhunter/common.js'
...
export async function applyDomainsRegion(cacheDir, code2domain) {
	const root = await getWikiPage('Domains', cacheDir, 'en')
		...
		tryWithContext('wiki domains', 'skipping', null, () => {
			mustForEachTableRow(
				...
				header => ({
					...
					location: mustFindCellIndex(header.children, /^location$/i),
				}),
