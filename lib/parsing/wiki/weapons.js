...
import { arrPushIfNew, arrSplitFn } from '#lib/utils/collections.js'
...
async function extractWeaponObtainSource(cacheDir, path) {
	...
	const sources = /**@type {import('#lib/genshin').GI_WeaponObtainSource[]}*/ ([])
	searchNodeWithClass(root, 'pi-item', (node, ancestors) => {
		...
		for (const chunk of chunks) {
			const text = chunk.map(getTextContent).join('').toLocaleLowerCase().trim()
			const source = guessWeaponObtainSource(text)
			if (source === '_unknown_') warn(`wiki page '${path}': unexpected obtain source line '${text}'`)
			else if (source !== '_ignore_') arrPushIfNew(sources, source)
		}
		return 'skip-children'
	})
...
/**
 * @param {string} text
 * @returns {import('#lib/genshin').GI_WeaponObtainSource|'_ignore_'|'_unknown_'}
 */
function guessWeaponObtainSource(text) {
	...
	if (text.endsWith(' fishing association')) return 'fishing'
	if (text.startsWith('sold by ')) return 'npc-shop'

	if (text === 'chests') return 'chests'
	if (text.startsWith('talk to ')) return 'quests'
	if (text.endsWith(' quest')) return 'quests'
	if (text.endsWith(' puzzle')) return 'puzzles'
	if (text.includes('stuck inside a stone')) return 'puzzles'
	...
	if (text.match(/^comes bundled with new \w+ characters$/)) return '_ignore_'
	return '_unknown_'
}
