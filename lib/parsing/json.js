...
/** @typedef {TextNodeB|TextNodeA|string} TextNodeInline */
	...
	for (let i = 0, textOffset = 0; i < pTexts.length; i += 2) {
		...
		if (pFormatRuns.length > 0) {
			...
			children = pFormatRuns.map((run, i, formatRuns) => {
				...
				const href = run.format.link?.url
				const isBold = run.format.bold

				let node = /**@type {TextNodeInline}*/ (text.slice(run.startIndex ?? 0, nextIndex))
				if (isBold) node = { b: node }
				if (href) node = { a: node, href }

				const otherKeys = new Set(Object.keys(run.format))
				otherKeys.delete('bold')
				if (otherKeys.size > 0) console.warn(`WARN: unexpected format keys: ${Array.from(otherKeys)}`)
...
function json_sliceFormatRuns(runs, start, len) {
	...
	for (let i = 0; i < runs.length; i++) {
		...
		if (startIndex < start) {
			if (next) {
				if (next.startIndex > start) res.push({ startIndex: start, format: run.format })
			} else {
