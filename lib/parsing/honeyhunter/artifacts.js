...
export async function extractArtifactsData(cacheDir, langs, fixes) {
	...
	for (const [id, artifact] of id2item) {
		...
		for (const { type, pieceId } of pieces) {
			...
			for (const lang of langs) {
				extras[lang] = { name: '', description: '', story: '' }
				const extra = extras[lang]
				const root = await getHoneyPage('art/a_' + pieceId, cacheDir, lang)

				tryWithContext(`artifact '${artifact.code}' #${id} ${type} name`, 'skipping', null, () => {
					const titleNode = mustFindNodeByClass(root, 'custom_title')
					extra.name = getTextContent(titleNode).trim()
				})

				tryWithContext(`artifact '${artifact.code}' #${id} ${type} descr`, 'skipping', null, () => {
				...
				tryWithContext(`artifact '${artifact.code}' #${id} ${type} story`, 'skipping', null, () => {
			...
			const piece = (artifact.pieces[type] ??= { name: {}, description: {}, story: {} })
			piece.name = makeLangMap(extras, 'name')
...
export function getArtifactSpecialGroupCodes(code2artifact) {
	...
	const groups = { '18%-atk': [], '20%-er': [], '25%-ph-atk': [] }
