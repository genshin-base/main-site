...
export async function extractItemsData(cacheDir, langs, fixes) {
	...
	await add('item/currency', 'currency') //fam_currency
	await add('item/ingredients', 'ingredient') //fam_forging_ingredient
	await add('item/character-ascension-material-jewel', 'character-material-jewel') //fam_char_jewel
	await add('item/character-ascension-material-elemental-stone', 'character-material-elemental-stone') //fam_char_stone
	await add('item/character-ascension-material-secondary-material', 'character-material-secondary') //fam_char_common
	await add('item/character-ascension-material-local-material', 'character-material-local') //fam_char_local
	await add('item/talent-level-up-material', 'character-material-talent') //fam_talent
	await add('item/weapon-ascension-material-primary', 'weapon-material-primary') //fam_wep_primary
	await add('item/weapon-ascension-material-secondary-material', 'weapon-material-secondary') //fam_wep_secondary
	await add('item/fish', 'fish') //mcat_28040101
...
function parseItemsTable(root, logPrefix) {
	...
	searchNodeWithClass(root, 'itemcont', (node, ancestors) => {
		tryWithContext(logPrefix, 'skipping', null, setLogPrefix => {
			...
			const [, type, id] = mustFindNodeHrefMatch(node, /^\/db\/item\/(i|h)_n?(\d+)/)
			...
			searchNodeWithClass(node, 'sea_item_used_by_char', node => {
				...
				for (const child of node.children) {
					...
					if (child.tag === 'a') {
						...
						if (!type || type === 'item') {
							const m = href.match(/^\/db\/item\/i_n?(\d+)/)
							if (m) {
								type = 'item'
