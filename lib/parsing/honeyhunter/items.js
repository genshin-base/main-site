...
export async function extractItemsData(cacheDir, langs, fixes) {
	...
	const add = (page, type) => parseItemsTablePage(page, cacheDir, langs, type, id2rawItem)
	...
	id2rawItem.forEach((raw, id) => {
		...
		for (const fix of fixes.items) {
			if (item.code === fix.code) fix._used = fix.fixFunc(item)
		}
...
export function groupItemsByCraft(items) {
	const code2item = /**@type {Map<string, import('#lib/parsing').ItemData>}*/ (new Map())
	for (const item of items) code2item.set(item.code, item)
	...
	for (const item of items) {
		...
		while (true) {
			...
			let newItem = null
			if (curItem.craftedFrom.length === 1)
				for (const src of curItem.craftedFrom) {
					if (src.count === 3) {
						newItem = mustBeDefined(code2item.get(src.code))
						break
					}
				}
			if (!newItem) break
...
function parseItemsTable(root, logPrefix) {
	...
	searchNodeWithClass(root, 'itemcont', (node, ancestors) => {
		tryWithContext(logPrefix, 'skipping', null, setLogPrefix => {
			...
			if (/^？+$/.test(name)) return // skipping items like '？？？'
