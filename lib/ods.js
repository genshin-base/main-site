 ...
 * @param {string} htmlZipFPath
 * @returns {Promise<BuildInfo>}
 */
export async function extractBuildsFromODS(odsFPath, htmlZipFPath) {
	let buildInfo = /**@type {BuildInfo|null}*/ (null)

	const htmlTablesMap = /**@type {Map<string, {get():Promise<Node>}>}*/ (new Map())
	const htmlZip = new StreamZip.async({ file: htmlZipFPath })
	for (const [name] of Object.entries(await htmlZip.entries())) {
		if (name.endsWith('.html'))
			htmlTablesMap.set(name.slice(0, -5), {
				get: async () => await parseXmlStream(await htmlZip.stream(name)),
			})
	}
 ...
 * @returns {Promise<BuildInfo>}
 */
async function processSpreadsheet(root, htmlTablesMap) {
	...
	const textItems = /**@type {TextStyledItem[]}*/ ([])
	for (const elem of Object.values(elementMap))
		for (const char of elem) {
			addStyledTextItems(textItems, char.notes)
			for (const role of char.roles) {
				addStyledTextItems(textItems, role.weapon)
				addStyledTextItems(textItems, role.artifact)
				addStyledTextItems(textItems, role.mainStats)
				addStyledTextItems(textItems, role.subStats)
				addStyledTextItems(textItems, role.talent)
				addStyledTextItems(textItems, role.tips)
			}
		}
 ...
 * @param {TextStyledItem[]} textItems
 */
